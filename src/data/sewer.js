...
export function sewerFaults() {
  ...
  // --- ROUND 115: THE FIRST SWORD ------------------------------------------
  //
  // "the closest room" is a claim about the map, so it is checked against the
  // map: the sword's chamber must be one step from the chamber the player
  // wakes in. And the guard must be standing over it rather than somewhere
  // else in the building -- a guard two chambers from what it guards is a
  // wandering slime.
  if (!p.marks.sword) {
    ...
    const step = Math.abs(sc.cx - wc.cx) + Math.abs(sc.cy - wc.cy);
    if (step > 1) out.push(`the first sword is ${step} chambers from the start, not the closest room`);
    if (step === 0) out.push('the first sword is in the chamber the player wakes in -- nothing is found');
  }
  if (!p.marks.guard) {
