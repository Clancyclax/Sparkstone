...
export function debuffStatsFragment(a) {
  ...
  return ` · ${Math.round(d.chance * 100)}% ${shown} ${d.duration}s`
    + (a.contagion ? ` · spreads to ${a.contagion.max} within ${fmtTiles(a.contagion.radius)}` : '');
}
...
function statsLineBase(a) {
  switch (a.template) {
    ...
    case 'aoeRing': {
      let s = `${a.base} dmg to all enemies within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown)}`;
      if (a.dot) s += ` · +${a.dot.dmgPerTick}×${a.dot.ticks} ${a.dot.label}`;
      return s;
    }
    // ---- ROUND 38 (sections 6.1-6.10) ----
    case 'bloomField': return `a field within ${fmtTiles(a.range)} restoring ${a.healPerSec} HP/s to you and allies for ${a.fieldDuration}s · ${fmtCd(a.cooldown)}`;
    case 'partyBuff': return `allies within ${fmtTiles(a.range)}: +${Math.round(a.partyDmgPct * 100)}% damage and +${a.partyPower} power for ${a.buffDuration}s · ${fmtCd(a.cooldown)}`;
    case 'aoeHealPulse': return `restores ${a.healAmount} HP (+1% per 1% cast speed) to you and allies within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown)}`;
    case 'weakenRing': return `-${Math.round(a.sunder.amount * 100)}% armor, -${Math.round(a.slowPct * 100)}% speed to enemies within ${fmtTiles(a.range)} for ${a.sunder.duration}s · ${fmtCd(a.cooldown)}`;
    ...
    case 'tauntPull': {
      const threat = (a.threatMult > 1)
        ...
        : '';
      return `draws up to ${a.tauntMax} enemies within ${fmtTiles(a.tauntRadius)} onto you for ${a.tauntDuration}s${threat} · ${fmtCd(a.cooldown)}`;
    }
    // ---- ROUND 49 ----
    case 'stealthVeil': {
    ...
    // ---- ROUND 48 ----
    case 'confuseTurn':
      return `turns up to ${a.maxTargets} enemies within ${fmtTiles(a.range)} against each other for ${a.confuseDuration}s · ${fmtCd(a.cooldown)}`;
    case 'fateReroll': {
      const what = {
    ...
    case 'abilityLock':
      return `stops the target acting for ${a.lockSeconds}s within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown)}`;
    ...
    case 'timeFreeze': return `freezes every creature within ${fmtTiles(a.freezeRadius)} for ${a.freezeDuration}s · ${fmtCd(a.cooldown)}`;
    ...
    case 'aura': {
      ...
      if (row) return `${row.label}: ${row.gained[row.gained.length - 1].adds}`;
    ...
    case 'activeSummon': {
      ...
      return `${what}: ${a.summonDmg} dmg every ${a.summonInterval}s within ${fmtTiles(a.summonRange)}, lasts ${life} · ${fmtCd(a.cooldown)}`;
    }
