...
import { CORPSE_CATEGORIES, corpseAbilitySpec } from './corpseAbilities.js';
...
// ===========================================================================
// ROUND 174 -- A RADIUS A PLAYER CAN ACT ON.
//
// The user, on an aura card: "Aura ability that's almost unintelligible. I
// think it's multiple effects just listed in order but it seems to be
// referring to information or context not available to the player."
//
// The clearest instance in the card he sent is "a stack every 2.2s within
// 104". 104 is WORLD UNITS -- the number the collision code works in -- and
// there is nothing anywhere in the game that tells a player what one of them
// is. A tile is 32 of them, so 104 is a bit over three tiles; the player can
// see tiles, and can see three of them, and cannot see 104 of anything.
//
// Rounded to the half tile and never below one: "within 3 tiles" is a
// distance, "within 3.25 tiles" is the same false precision in a new unit, and
// "within 0 tiles" is nonsense for a field that plainly has a size.
export function fmtTiles(units) {
...
import { kitClassFor, classPull, tallyKit, abilityRoles, primaryQuota as primaryQuotaFor, essenceLeanScore } from './buildClasses.js';
// ROUND 115 -- the 44 auras, at five ranks apiece. See data/sparkstone_auras.csv.
import { AURAS, AURA_KEYS, aurasFor, specAtRank as auraAtRank, auraEffectClauses } from './auras.js';
...
// ROUND 200 -- mounts: a bonded familiar you can ride from Bronze.
import { MOUNT_CAP, MOUNT_CHANCE, isMountable, mountRidersAt, mountStateFor } from './mounts.js';
// ROUND 200 -- what a vehicle essence is for. See that file's header.
import { isVehicleEssence, vehicleBoonsFor, VEHICLE_BOON_BY_KEY } from './vehicleEssence.js';
...
// ROUND 206 -- the kit's synergy plan. See kitSynergy.js.
import { resolveKitSynergy } from './kitSynergy.js';
import { rollBuffs, formatBuff, ELEMENT_TYPES, DAMAGE_TYPES } from './stats.js';
import { ESSENCE_MOTIFS } from './essenceMotifs.js';
// ROUND 202 (item 3) -- the rank ladders, and the guard that keeps the filler
// they replaced from coming back. See rankLadders.js.
import { ladderFor, fillerReason, FILLER_PATTERNS } from './rankLadders.js';
// ROUND 202 (item 2.8) -- which essence leans which way, and how likely it is
// to teach it. See proficiencies.js.
import { PROFICIENCIES, profGrantFor, profForWeapon, masteryWeaponFor,
  ESSENCE_PROFICIENCY, ESSENCE_WEAPON } from './proficiencies.js';
// ROUND 203 (item 5) -- transcendent damage's rarity and its gates. The
// mechanics live in transcendent.js; this file only decides who gets one.
import {
  TRANSCENDENT, TRANSCENDENT_ODDS, GATE_KEYS as TRANSCENDENT_GATE_KEYS,
  TRANSCENDENT_GATES, gateClause,
} from './transcendent.js';

// ROUND 201 -- the canon layer. See abilityCanon.js; nothing here generates
// differently because of it, but a generated spec may now DECLARE a shape the
// books use and the card will print it.
import {
  triggerIcd, canonHours, withCanonCooldown, COOLDOWN_VARIES, VARIES_RULES,
  TARGET_SCOPES, TARGET_SCOPE_KEYS, scopeWho, scopeAllowsSelf,
  TRANSFORMATIONS, TRANSFORMATION_KEYS, transformAtRank,
  // ROUND 203 (item 1) -- the per-rank figures, for the socket that now rolls
  // a transformation.
  TRANSFORM_BY_RANK,
...
import { SCALED_FIELDS, SCALED_NESTED, abilityScale } from './essenceRank.js';
// ROUND 204 (item 7) -- the per-level steps. Imported here so `rankScaled`
// (what the CARD prints) and `_scaledAbility` (what the runtime USES) apply
// the same ones; see the note above ABILITY_DAMAGE_MULT, which has warned
// about exactly this split since round 116 and was right.
import {
  MINOR_COOLDOWN_PER_LEVEL, MINOR_REACH_PER_LEVEL, MINOR_REACH_FIELDS,
  MINOR_COST_PER_LEVEL, MINOR_COST_FLOOR,
} from './abilityLevels.js';
...
import { summonCreatureFor, summonCreatureForSocket, SUMMON_CREATURES, SUMMON_CREATURE_BY_FAMILY,
  // ROUND 207 -- the animal a familiar's name may honestly promise.
  namePromisesWrongBeast } from './summonCreatures.js';
...
export function resolveConfluenceName(sortedIds) {
  ...
  // =======================================================================
  // ROUND 205 (item 4) -- TWO WEAPONS ARE AN ARSENAL, BY RULING.
  //
  //   "Lets says 2+ weapon essences always result in the arsenal confluence."
  //
  // Asked whether that was already true, the measurement was: 4,088 two-weapon
  // trios, 2,918 of them (71.4%) already resolving to Arsenal, and Arsenal the
  // ONLY name in the ninety-nine-name catalogue whose vocabulary is weapons.
  // So the derivation was most of the way here on its own and the remaining
  // 28.6% were the scorer reaching for a name that knew nothing about the
  // trio. This makes the concentration a decision instead of an artefact, and
  // round 205's item 4 gives the name a mechanical identity to deserve it.
  //
  // AFTER THE CANON, not before. The sheet's ruling stands wherever the sheet
  // has ruled -- round 117's rule, and the reason this line is here rather
  // than at the top of the function. It costs exactly one trio:
  // Cold + Hammer + Sickle, which the sheet names Desolate.
  // =======================================================================
  if (countWeaponEssences(sortedIds) >= 2) return 'Arsenal';
  return deriveConfluenceName(sortedIds);
}

/** How many of these essence ids are weapon essences. One line, named, because
 *  three places now ask it and a re-derived `filter` in each is how the two
 *  directions of a rule start disagreeing. */
export function countWeaponEssences(ids) {
  return (ids || []).filter(id => ESSENCE_PROFICIENCY[id]).length;
}
...
export function confluenceInnateAbility(confDef, usedNames = null, spine = null, buildSeed = '',
  ...
  barrierAllowed = true,
  // ROUND 203 -- the rank this kit is being built at, for the transcendent
  // roll in `applyEssenceFlavour`. Defaulted to iron, the rarest band, so a
  // caller that predates it is unaffected in the safe direction.
  kitRank = 'iron') {
  const c = confDef;
  ...
  if (cat) {
    // ROUND 203 -- a confluence innate rolls at the kit's rank like everything
    // else. `opts` is not in scope here; the confluence builder takes its rank
    // from the same place its spine comes from.
    applyEssenceFlavour(spec, c, null, cat, roll, c.base,
      { keepText: true, spine, kitRank: kitRank || 'iron' });
...
// ===========================================================================
// ROUND 174 -- THE STONE IS THE VERB, AND THE COMPOSER COULD NOT HEAR IT.
//
// The user:
//   "With the wolf essence and swift stone I've got an ability that summon a
//    whip to follow me around floating in the air. (A floating whip has
//    nothing to do with speed, or wolves)"
//   "A wolf stone of the cage gave me a movement speed ability. While that is
//    somewhat wolf themed, it's the opposite of what a cage stone should be
//    giving you."
//
// MEASURED ACROSS 300 KITS -- 4,736 abilities, each classified against the
// stone in its own socket:
//
//     on the stone's bias list      191    4.0%
//     on its AVOID list               6    0.1%
//     NEITHER                     4,539   95.8%
//
// -- and the sample includes his exact report, twice:
//     Cage(guard) -> movement_dash : Passage of the Muzzle
//     Cage(guard) -> movement_dash : Needle-Bone Stride
//
// WHY. Round 48 built the bias system and its own note says "100% of the
// category pressure on a socket came from the awakening stone". Round 109 then
// replaced the menu with a COMPOSER, and wrote the honest line about it: "The
// composer decides WHAT is offered; the lever system still decides what is
// allowed." The lever system is `effectiveMotif(essDef, spine).levers` -- the
// ESSENCE's motif, and nothing else. So from round 109 the stone stopped
// contributing anything to what an ability DOES; all it kept was a vote on
// which descriptor row the finished composition got LABELLED with, through
// `stonePrefer`, and a veto in `tryCat` that a composed row rarely meets.
//
// A field that is written and read by nothing is fault class 1. This one is
// worse: it is read, by a path nothing picks from any more.
//
// SO A STONE BRINGS A LEVER. One each, its family's, joined to the essence's
// motif when the socket composes -- which makes the two halves of a socket say
// what the user says they say: the essence is the noun (a wolf), the stone is
// the verb (quickly / holding it still). A Wolf stone of Swift composes from
// the wolf's levers PLUS `swift`; a Wolf stone of Cage composes from the same
// levers plus `muzzle`, and muzzle is the lever that takes the tempo away.
//
// CHOSEN BY WHAT THE FAMILY DOES, not by its name -- the same rule the round
// 173 hit-effect table follows. Where two families do the same thing they
// share a lever; there are 29 families and 21 levers, and inventing a
// distinction so every family could have its own would be inventing it.
/** ROUND 174 -- how often a socket's TWIST is its stone's lever rather than
 *  its essence's. [numerator, denominator]; see applyEssenceFlavour for what
 *  the remaining third is protecting. */
export const STONE_LEVER_ODDS = [2, 3];

/** ROUND 174 -- the two POLARITIES a stone's lever may not reverse: an essence
 *  with no harm in its charter may not be given harm by its stone, and an
 *  essence with no healing may not be given healing. Round 52 states both, one
 *  in each direction -- "renew may produce nothing that harms" and "a linger
 *  essence still rots but never mends" -- and those two sentences are the
 *  whole of what this set is for. Everything else a stone brings is allowed:
 *  a healer who moves faster is not a contradiction. See applyEssenceFlavour. */
const POLARITY_FAMILIES = new Set(['damage_direct', 'damage_overtime', 'heal']);

export const FAMILY_LEVER = {
  // weapons: what the weapon is FOR
  blade: 'raw', bludgeon: 'raw', polearm: 'reach', ranged: 'reach', force: 'raw',
  // the defensive family, and the one the user caught: a cage HOLDS
  guard: 'muzzle',
  // creatures: what having one with you is worth
  beast: 'call', smallbeast: 'swift', flyer: 'swift', aquatic: 'renew',
  serpent: 'linger',
  // elements: how each one actually hurts or helps
  fire: 'burst', water: 'mend', air: 'shift', earth: 'anchor', cold: 'muzzle',
  storm: 'chain', light: 'absolve', dark: 'stealth',
  // the vital families
  life: 'renew', death: 'siphon', blood: 'siphon', alchemy: 'mend',
  // and the abstract ones
  mind: 'fate', motion: 'swift', space: 'shift', order: 'anchor',
  identity: 'turn', craft: 'mend',
};
...
export const ABILITY_CATEGORIES = [
  ...
  // ===== ROUND 203 (item 1) -- THE SELF-TRANSFORMATION ====================
  //
  // Round 201 read [Specious Sorcerer], [Counterfeit Combatant] and [Instant
  // Adept] out of canon, found they were ONE ability with three swaps, built
  // the template and proved the card renders it -- and then nothing in the
  // game could roll one. The vocabulary existed and nothing used it, which is
  // this project's oldest fault class wearing its best clothes.
  //
  // This is the socket that uses it. `rareOnly` because a transformation is
  // the largest single thing an ability can do -- an attribute, a proficiency,
  // a raised pool and an ongoing regen, all at once -- and because canon gives
  // them to a Charlatan and an Adept rather than to everyone. It rides the
  // rare seat the stacking boons already use (round 75's one-in-forty door),
  // so it costs the kit shape nothing.
  //
  // `sheetTypes` is empty for the same reason `attr_boost`'s is: there is no
  // art sheet for "you briefly become a better version of yourself", and a
  // sheet type it cannot fill would put a missing-texture box on the card.
  { key: 'self_transformation', kind: 'active', category: 'buff', template: 'transform',
    rareOnly: true, sheetTypes: [],
    names: ['Borrowed {A}', 'The {A} You Are Not', 'Counterfeit {A}', 'Instant {A}'] },
  ...
  // ===== ROUND 164 (item 1.1) -- REACHING WHAT CANNOT BE REACHED ==========
  //
  // THE USER: "As an affliction focused build develops they should find their
  // passives, auras, or abilities give them the ability to afflict enemies who
  // are naturally immune to their afflictions. This might be abilities that
  // allow a specific affliction or broader changes."
  //
  // SIX NARROW ONES AND ONE BROAD ONE, and the split is the point. A narrow
  // grant answers ONE question -- "my poisons work on things that do not
  // breathe" -- and is the ordinary reward for going deep on poison. The broad
  // one answers all of them and is deliberately the rarest thing on this list,
  // because an affliction build that can afflict anything has stopped having a
  // matchup and the matchups are what make the subtypes worth having.
  //
  // WHY EACH ONE IS THE AFFLICTION'S OWN TAG. These are not new damage types
  // or a resistance ladder: each says "the immunity does not apply to me" for
  // exactly one of the six words the user used, which is the same vocabulary
  // `SUBTYPE_IMMUNE_TAGS` is written in. One list of six, read from both ends.
  //
  // The names carry the flavour and the description states the mechanic, which
  // is this project's standing rule -- a player reading "Something In It That
  // Should Not Take" should learn from the line under it that their poisons
  // now land on the bloodless.
  { key: 'virulence_poison', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['poison'], sheetTypes: [],
    names: ['Something In It That Should Not Take', 'The Wrong Kind Of Poison',
      'It Finds A Way In', 'Corrosive Intent', 'No Blood Required',
      'The Patient Solvent'] },
  { key: 'virulence_bleed', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['blood', 'wounding'], sheetTypes: [],
    names: ['Opens Anything', 'The Wound That Is Not A Wound', 'Severance',
      'It Still Comes Apart', 'The Unmaking Cut', 'Faultline'] },
  { key: 'virulence_curse', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['curse'], sheetTypes: [],
    names: ['A Word For Things Without Names', 'The Curse That Does Not Need A Soul',
      'Spoken At It Anyway', 'The Binding Syllable', 'Ill-Wished All The Same',
      'The Nameless Malice'] },
  { key: 'virulence_burn', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['burning'], sheetTypes: [],
    names: ['Burns What Burns', 'The Hotter Thing', 'Fire Under Water',
      'It Will Catch', 'The Impossible Ember', 'Kindling From Stone'] },
  { key: 'virulence_holy', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['holy'], sheetTypes: [],
    names: ['Judgement Without Appeal', 'It Answers To Something',
      'The Sentence Stands', 'Hallowed Regardless', 'No Exemption',
      'The Light Does Not Ask'] },
  { key: 'virulence_unholy', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesTags: ['unholy'], sheetTypes: [],
    names: ['Damnation Has No Exceptions', 'The Deeper Rot', 'Already Yours',
      'It Takes Anyway', 'The Second Death', 'Claimed'] },
  // The broad one. See the note above for why there is exactly one.
  { key: 'virulence_all', kind: 'passive', category: 'affliction', template: 'virulence',
    piercesAll: true, rarity: 'rare', sheetTypes: [],
    names: ['Nothing Is Proof Against You', 'The Universal Solvent',
      'Everything Can Be Made To Suffer', 'No Constitution Survives This',
      'The Great Leveller', 'Immunity Is A Word For Not Yet'] },
...
// ROUND 189 -- the five corpse categories join the table so every lookup by
// key finds them, and are kept OUT of the two lists a socket is dealt from:
// they reach a kit only as the six death-shaped essences' signatures. See
// corpseAbilities.js.
ABILITY_CATEGORIES.push(...CORPSE_CATEGORIES);
export const ABILITY_CATEGORY_BY_KEY = Object.fromEntries(ABILITY_CATEGORIES.map(c => [c.key, c]));
const ACTIVE_CATEGORIES = ABILITY_CATEGORIES.filter(c => c.kind === 'active' && !c.corpse);
const PASSIVE_CATEGORIES = ABILITY_CATEGORIES.filter(c => c.kind === 'passive' && !c.corpse);
...
export function composedCategoryFor(comp, opts = {}) {
  ...
  // ROUND 190 (update 8) -- and so is the WEAPON a special attack requires.
  // The row is cached by key; keyed without it, the first essence to compose a
  // weapon special (a Whip's) fixed `requiresWeapon` for every later essence
  // that composed the same shape -- a Sickle's "Weaken Armor" asked for a whip.
  const reqW = (comp.type === 'special' && comp.leadHostile && opts.weaponId && opts.weaponId !== 'unarmed')
    ? opts.weaponId : null;
  const key = `cmp_${comp.effects.join('-')}_${comp.delivery}_${template}${reqW ? `_${reqW}` : ''}`;
...
function composedTriggerFields(pc) {
  ...
  // ROUND 201 -- `triggerIcd` rather than `trigger.cooldown || 6`.
  //
  // [Karmic Warrior] is written "Cooldown: None." and fires "whenever you are
  // affected by a harmful effect, even if that effect is wholly negated". The
  // `||` above turned a deliberate zero into six -- the classic falsy-but-
  // legitimate-value fault -- so a free reactive was unsayable. The fix is
  // narrow on purpose: a zero survives only for a trigger kind written to be
  // free (FREE_TRIGGERS), so nothing composed before this round changes.
  return { trigger, effect, isTriggered: true, cooldown: triggerIcd(trigger, 6) };
}
...
export const WEAPON_BY_IDENTITY = {
  ...
  Hook: 'whip', Sceptre: 'staff',
  // ===== ROUND 215 -- FOUR MORE THINGS THAT ARE NOT WEAPONS ===============
  //
  // The user, on a Fork stone socketed into a Plant essence:
  //
  //   "An awakening stone of the fork on a plant essence granted a spear
  //    weapon skill. This is dumb. It A) shouldn't be a weapon skill and
  //    B) forks and plants dont equal spears."
  //
  // ...and, separately: "Rake is not equal to scythe."
  //
  // Both were here: `Fork: 'spear'` and `Rake: 'scythe'`. Round 120 already
  // settled this exact question for `Spike: 'javelin'` -- "a spike is not a
  // weapon" -- and the reasoning transfers whole. A fork and a rake are
  // things you work ground with. The tines resemble a spear the way a ladder
  // resembles a portcullis.
  //
  // `Shovel: 'hammer'` and `Trowel: 'dagger'` were not reported and are the
  // same mistake: garden tools given a weapon identity because they are
  // roughly the right shape. They go with the other two rather than waiting
  // to be found one screenshot at a time, which is the pattern this project
  // keeps paying for. Hook stays -- a war hook and a boat hook are both real
  // reach weapons, and the resemblance is not the reason it is here.
  //
  // Removing the identity is the whole fix, exactly as it was for Spike:
  // `weaponIdentityOf` returns null, `weaponForAffinity` refuses the category
  // outright, and the socket spends its seat on something the essence can
  // actually do.
  // bludgeon
 ...
 */
export function weaponForAffinity(stone, essDef, kitWeapon = null) {
  // ROUND 190 (update 8.1) -- "A weapon essence can only require use of
  // weapons of its own type." A Sickle essence's abilities are Sickle
  // abilities whatever stone is in the socket; the stone's weapon gets its say
  // through the kinship below (weaponAffinity's countsAs / matched speed).
  const own = weaponIdentityOf(essDef);
  if (own && own !== UNARMED_ID) return own;
  // ===== ROUND 215 (item 4.1) -- A WEAPON THE KIT CANNOT USE ==============
  //
  // The user, on the same Fork-on-Plant ability:
  //
  //   "with a weapon essence already in the kit and no confluence granting
  //    bonuses across spears and daggers the player is now forced to abandon
  //    part of their kit."
  //
  // Exactly so, and it is the larger half of the fault. Removing Fork's
  // identity fixes the reported ability and nothing else: a Sword stone in a
  // Plant socket, in a kit whose other essence is Knife, still hands out
  // sword passives to a player committed to daggers. The ability is correct,
  // well-named, and dead on arrival, because a kit holds one weapon at a
  // time and this one was never going to be it.
  //
  // So a STONE's weapon identity is honoured only when the kit has not
  // already committed to a different one. `kitWeapon` is the weapon the kit's
  // own ESSENCES declare -- an essence is what the player chose, a stone is
  // what the socket rolled, and when they disagree the choice wins.
  //
  // Returning null rather than substituting the kit's weapon is deliberate:
  // `cat.isWeaponAffinity` already refuses a category with no weapon, so the
  // socket spends its seat on something else instead of producing a sword
  // passive wearing a dagger's name.
  const fromStone = weaponIdentityOf(stone);
  if (fromStone && kitWeapon && fromStone !== kitWeapon && fromStone !== UNARMED_ID) return null;
  return fromStone || own || null;
}

/** The weapon a KIT has committed to, from its essences alone, or null.
 *
 *  Null when the kit names none (nothing to conflict with) and null when it
 *  names two or more -- a player holding Sword and Knife has deliberately
 *  built across weapons, and refusing stone affinities in that kit would be
 *  the rule punishing the build it was written to protect. */
export function kitWeaponOf(essDefs) {
  const found = new Set();
  for (const d of (essDefs || [])) {
    const w = weaponIdentityOf(d);
    if (w && w !== UNARMED_ID) found.add(w);
  }
...
// ROUND 190 (update 9) -- "Like Auras, having more than 1 perception ability
// requires a very rare passive ability. This ability will only ever occur with
// an omen, vision, or lurker essence and when paired with an appropriately
// thematic awakening stone."
export const MULTI_PERCEPTION_ESSENCES = ['essOmen', 'essEye', 'essLurker'];
export const MULTI_PERCEPTION_STONES = ['stoneOmen', 'stoneOmens', 'stoneVision', 'stoneEye', 'stoneLurker',
  'stoneEcho', 'stoneSmoke', 'stoneDark', 'stoneHunt', 'stoneFocus'];
export function hasMultiPerceptionPassive(essenceIds, stoneIds = []) {
  const ids = essenceIds.filter(Boolean);
  if (ids.length < 3) return false;
  if (!ids.some(id => MULTI_PERCEPTION_ESSENCES.includes(id))) return false;
  if (!(stoneIds || []).some(id => MULTI_PERCEPTION_STONES.includes(id))) return false;
  return stableHash(ids.slice().sort().join(',') + '|multiperception') % 12 === 0;
}

// Formats a cooldown for stats lines -- long cooldowns read in minutes.
//
// ROUND 201 -- the dense line needs the same three bands the card grew this
// round, in its own abbreviated voice. It was printing "240m cd" for a canon
// day and "variess cd" for a cooldown computed at use, because it assumed a
// number and every cooldown had been one. Deliberately NOT delegated to
// `cooldownPhrase`: that function writes sentences for the card ("4 hours.")
// and this one writes tokens for a line the player scans ("4h cd"), and
// making one serve both would give the card abbreviations or the line
// sentences.
function fmtCd(cd, spec = null) {
  // ROUND 203 -- the compact stats line says what the card says. The floor is
  // in `cd` and is true, but "4s cd" on an ability whose cooldown is the time
  // it just gave back would be the stats line and the card disagreeing about
  // the same number, which is worse than either being vague.
  if (spec && spec.variesCooldown && VARIES_RULES[spec.variesCooldown]) return 'varies cd';
  if (cd === COOLDOWN_VARIES) return 'varies cd';
  if (typeof cd !== 'number' || !isFinite(cd)) return '';
  if (cd <= 0) return 'no cd';
  if (cd >= 5400) { const h = Math.round(cd / 360) / 10; return `${h % 1 === 0 ? h : h.toFixed(1)}h cd`; }
...
export function assignAbilityCost(spec, stone) {
  ...
  // ROUND 190 (6.3) -- a spammable ability is cheap to press once; what it
  // costs is the streak (spamCostPer, paid at cast time).
  if (spec.spammable && spec.cost) {
    spec.cost = { ...spec.cost, amount: Math.max(2, Math.round(spec.cost.amount * 0.4)) };
    if (typeof spec.cooldown === 'number' && spec.cooldown > SPAM_CD_MAX) spec.cooldown = SPAM_CD_MAX;
  }
}
...
export function appendDebuffClause(spec) {
  ...
  if (spec.contagion) {
    spec.desc += ` It is catching: ${def.label.toLowerCase()} finds up to `
      + `${spec.contagion.max} more within ${fmtTiles(spec.contagion.radius)} on its own.`;
  }
  return spec;
}
...
export function debuffStatsFragment(a) {
  // ROUND 206 -- the seeded condition is on this line too.
  //
  // Round 57's lesson, in the other direction: a marquee ability once carried
  // "35% dulled 6.6s" in its stats line and not a word about it in its
  // description, and the user's wording rule exists to stop exactly that
  // split. A synergy clause that states the mechanic in prose and leaves the
  // numbers line silent is the same split facing the other way, so the seeded
  // condition is folded in here rather than being described and never counted.
  const own = debuffFragmentFor(a, a && a.debuff, a && a.contagion);
  const seeded = debuffFragmentFor(a, a && a.synergyDebuff, null);
  return own + seeded;
}

function debuffFragmentFor(a, d, contagion) {
  // ROUND 113 -- `conditionDef`, not `DEBUFFS[...]`.
  //
  // DEBUFFS holds the thirty-two AUTHORED conditions; everything composed or
  // authored-in-the-CSV lives in the registry behind `conditionDef`, which
  ...
  // ROUND 190 (bug 1) -- a bracketed name alone says nothing about what it
  // does, so the chance is worded as a chance and the condition's own blurb
  // follows it.
  const what = def.blurb ? ` (${def.blurb})` : '';
  return ` · ${Math.round(d.chance * 100)}% chance of ${shown} for ${d.duration}s${what}`
    + (contagion ? ` · spreads to ${contagion.max} within ${fmtTiles(contagion.radius)}` : '');
}
...
const RESIST_PHRASES_ACTIVE = [
  ...
  (n, e) => `${cap(e)} damage bites ${n}% less deeply while it lasts.`,
  // ROUND 204 (item 2.6.1) -- "Should say nature damage to be clear." The
  // element word is a bare adjective here, so every phrasing that left it
  // dangling now names what it is an adjective for.
  (n, e) => `For its duration you shrug off ${n}% of incoming ${e} damage.`,
...
const RESIST_PHRASES_PASSIVE = [
  ...
  (n, e) => `${cap(e)} damage is permanently ${n}% less dangerous to you.`,
...
export function rankAspectsFor(a) {
  ...
  // ===== ROUND 205 (item 1.1) -- EXCEPT THE MASTERY LINE ==================
  //
  // The user: "Ability is silver rank but doesn't have a bronze or silver
  // effect", on a Silver-rank [Hammer Mastery].
  //
  // The rule below is right about the GENERIC ladders and was applied to the
  // one that is not generic. An attribute passive's authored table answers
  // "what does deepening this attribute buy me"; the mastery ladder answers
  // "what does knowing this weapon buy me", and round 204 made that ladder the
  // one place a weapon essence's three promises live. The proficiency sweep
  // stamps `prof` onto whichever passive the slot already holds, and when that
  // passive happened to be an `attrBoost` the line was silently deleted --
  // measured at 11 of 347 masteries, every one of them a card that promised
  // three ranks of weapon training and printed none.
  //
  // Nothing collides: the riders grant attribute stats and the mastery rungs
  // grant `masteryDmgPct` / `masteryConjures` / `masteryCheaper`, and the card
  // now prints both under the rank each is true at.
  if (a.riders && a.riders.length && !a.prof) return [];
  ...
  // ROUND 202 -- the two striking ladders moved to rankLadders.js and became
  // POOLS. As fixed rows they put "+10% critical chance with this ability" on
  // 33.8% of every kit in the game -- "one ability offered twice" (round 134)
  // between abilities rather than inside one. `tags.strikes` is computed here
  // and handed over, because it is a narrower test than `tags.offensive` for
  // the reason that flag's own comment gives.
  // ===== ROUND 202 (item 3) -- THE POTENCY LADDER IS GONE ==================
  //
  // What stood below this line was four rungs of "the guard holds a tenth
  // further than it did" for anything that does not strike, plus two FIXED
  // rows for anything that does. All six are `ladderFor` now.
  //
  // The generic ladder was a percentage of a figure the card prints four lines
  // above, which round 103 had already ruled out once ("10% stronger than
  // what?") and which round 190 put back on the card when it rebuilt the card
  // out of these labels. Two measurements settled it: 27.6% of the abilities
  // carrying one printed IDENTICAL numbers at iron and gold, because the field
  // the label claimed to scale is not in SCALED_FIELDS at all; and 53.5% of
  // the abilities told "it comes back 10% sooner" were passives with no
  // cooldown at all.
  //
  // `ladderFor` gives the same four ranks real mechanics, chosen by what the
  // ability IS, picked from a pool so two abilities of a kind do not read
  // alike, and DROPS any rung whose field the spec does not carry. A rank with
  // nothing honest to add now adds nothing, which the card already renders as
  // no line at all.
  //
  // The potency itself did not go anywhere: SCALED_FIELDS still scale every
  // rank and the card prints the scaled figures. What went away is the
  // sentence announcing that they did.
  return ladderFor(a, { ...tags, strikes }, w);
}
...
/**
 * ROUND 204 (item 7) -- THE PER-LEVEL STEPS, in one place both readers call.
 *
 * Cooldown down, reach up, and now cost down. Applied to a clone that already
 * carries the rank scaling, because the level is a step INSIDE the rank.
 *
 * `_scaledAbility` in WorldScene calls this too. That is the whole point: the
 * comment above ABILITY_DAMAGE_MULT has said since round 116 that "a
 * multiplier applied to one and not the other is a card that lies about the
 * ability under it", and it was describing what this function fixes -- the
 * card was printing every number as if the ability sat at level 0, so at the
 * top of a rank it understated real damage by about half.
 */
export function applyLevelSteps(out, level = 0) {
  if (!out || !(level > 0)) return out;
  if (typeof out.cooldown === 'number' && out.cooldown > 0) {
    out.cooldown = Math.round(out.cooldown * (1 - level * MINOR_COOLDOWN_PER_LEVEL) * 100) / 100;
  }
  for (const f of MINOR_REACH_FIELDS) {
    if (typeof out[f] === 'number' && out[f] > 0) out[f] = Math.round(out[f] * (1 + level * MINOR_REACH_PER_LEVEL));
  }
  // The cost is a nested object, so it is cloned rather than edited: the spec
  // handed in may be the kit's own, and a level step that mutated it would
  // make the discount permanent and then compound it on the next read.
  if (out.cost && typeof out.cost.amount === 'number' && out.cost.amount > 0) {
    const f = Math.max(MINOR_COST_FLOOR, 1 - level * MINOR_COST_PER_LEVEL);
    out.cost = { ...out.cost, amount: Math.max(1, Math.round(out.cost.amount * f)) };
  }
  return out;
}

/**
 * ROUND 204c -- THE BOON LADDER'S STEPS, in one place both readers call.
 *
 * They were written into `_scaledAbility` alone, so the runtime applied them
 * and the card did not: the card printed "It goes up instantly" one line above
 * "0.8s cast". That is round 204's item 7 exactly, made again, one round
 * later, by me -- which is the answer to the user's question about whether
 * this is understanding or application. Understanding the rule is not the same
 * as having one place that enforces it, and this is the place.
 *
 * `aspects` is `rankAspectsAt(spec, rank)` -- the rungs actually reached.
 */
export function applyBoonSteps(out, aspects) {
  if (!out || !aspects || !aspects.length) return out;
  let durMult = 1, magMult = 1, cheaper = 0, permanent = false, instant = false;
  for (const x of aspects) {
    if (x.boonDurationMult) durMult *= x.boonDurationMult;
    if (x.boonMagnitudeMult) magMult *= x.boonMagnitudeMult;
    if (x.boonCheaper) cheaper = Math.max(cheaper, x.boonCheaper);
    if (x.boonPermanent) permanent = true;
    if (x.boonInstant) instant = true;
  }
  if (instant && out.castTime > 0) out.castTime = 0;
  if (durMult !== 1) {
    for (const f of ['buffDuration', 'duration']) {
      if (typeof out[f] === 'number' && out[f] > 0) out[f] = Math.round(out[f] * durMult * 10) / 10;
    }
  }
  if (magMult !== 1) {
    // Named rather than blanket-scaled: `powerMult` is a multiplier around 1,
    // so multiplying it directly would turn +27% into +59% rather than +34%.
    for (const f of ['critChanceBonus', 'buffAmount', 'speedPct', 'dmgPct', 'armorPct']) {
      if (typeof out[f] === 'number') out[f] = Math.round(out[f] * magMult * 1000) / 1000;
    }
    if (typeof out.powerMult === 'number' && out.powerMult > 1) {
      out.powerMult = Math.round((1 + (out.powerMult - 1) * magMult) * 1000) / 1000;
    }
  }
  if (cheaper && out.cost && typeof out.cost.amount === 'number') {
    out.cost = { ...out.cost, amount: Math.max(1, Math.round(out.cost.amount * (1 - cheaper))) };
  }
  // "No longer runs out on its own": a very long clock rather than a second
  // code path for a buff with no timer, so every tick, expiry and dispel keeps
  // working exactly as it does for every other buff.
  if (permanent) {
    for (const f of ['buffDuration', 'duration']) {
      if (typeof out[f] === 'number' && out[f] > 0) out[f] = BOON_PERMANENT_SECONDS;
    }
  }
  return out;
}

/** ROUND 204c -- an hour is "no longer runs out on its own" in every practical
 *  sense, and keeps the buff on the same timer and dispel path as every other. */
export const BOON_PERMANENT_SECONDS = 3600;

export function rankScaled(a, rank, level = 0) {
  ...
  // ROUND 204 -- and a TRANSFORMATION, for the same reason: its whole state
  // lives in a per-rank table, so a line built without the rank would print
  // the iron figures to a gold character.
  if (a && a.template === 'transform' && a.transform) {
    a = { ...a, _transformRank: rank };
  }
  ...
  const lvl = Math.max(0, Math.min(9, Math.round(level || 0)));
  if (!potency && !lvl) return applyBoonSteps(abilityDamageScaled(a), aspects);
  const mult = abilityScale(lvl) * (1 + potency);
  const out = { ...a };
  ...
  return applyLevelSteps(applyBoonSteps(abilityDamageScaled(out), aspects), lvl);
}
...
function bindFamiliarCreature(spec, essDef, opts = {}) {
  ...
  if (prof) {
    ...
    // ROUND 200 -- AND WHETHER IT CAN BE RIDDEN.
    //
    // "These can be a later tier (bronze or higher) of a permanent summon."
    // The bonded familiar IS this game's permanent summon, so a mount is not a
    // new ability -- it is this one, from Bronze, for a third of the creatures
    // there is art of. See src/data/mounts.js for the whole argument; what is
    // decided HERE is only the flag, and it is decided here because this is
    // the one line that knows which creature the familiar turned out to be.
    //
    // STAMPED OFF A STABLE HASH, NOT A ROLL. The kit is rebuilt whenever a
    // stone moves, so a coin flipped at build time would give a player a mount
    // that came and went while they socketed something unrelated -- and would
    // make MOUNT_CAP meaningless, since the count is taken off the built kit.
    // The hash is over the ability's own identity, so the answer is the same
    // every time the same ability is generated.
    if (isMountable(prof.family)) {
      const h = stableHash(`${spec.key || spec.id || spec.name || ''}|mount|${prof.family}`);
      if ((h % 1000) / 1000 < MOUNT_CHANCE) {
        spec.mount = true;
        spec.mountFamily = prof.family;
      }
    }
    return;
  }
...
export function statsLineFor(a) {
  ...
  const need = a.requiresWeapon
    // ROUND 204 -- "requires a Axe in hand". One article, chosen by the noun
    // rather than assumed, because the roster has an Axe, an Ice-brand and an
    // Urumi in it and all three read as a mistake with "a".
    ? `requires ${anArticle((WEAPONS[a.requiresWeapon] || {}).name || a.requiresWeapon)} in hand · ` : '';
  const line = need + statsLineBase(a) + leverStatsRider(a) + debuffStatsFragment(a) + cast;
  const full = (a.kind === 'active' && a.cost)
    ? `${line} · ${a.cost.amount} ${a.cost.type}${a.bloodSurrogate ? ' (or blood)' : ''}`
    : line;
  return dropBrokenSegments(full);
}

/**
 * ROUND 201 -- a segment that says `undefined` or `NaN` is dropped.
 *
 * Found by writing the ten canon transcriptions out as specs (abilityCanon.js)
 * and reading their cards back: a spec that does not carry a field some
 * template's fragment expects produced "undefined dmg" and "NaN% chance"
 * straight onto the player's card. Reference rows are not the real risk --
 * a GENERATED ability that misses a field does the same thing, and this line
 * is a place the project has shipped such a fault before.
 *
 * Dropped rather than zeroed: "0 dmg" is a claim about the ability and is
 * false, while a missing segment is merely a segment that is not there. If
 * dropping leaves nothing at all, the line says so rather than rendering as
 * an empty string that reads like a bug.
 */
export function dropBrokenSegments(line) {
  const parts = String(line || '').split(' · ')
    // No trailing word boundary: the fragments append units, so a missing
    // duration renders as "for undefineds" and `\bundefined\b` does not match
    // it. That was the first version of this line and it let half the cases
    // through.
    .filter(p => p && !/\bundefined|\bNaN/.test(p));
...
function leverStatsRider(a) {
  ...
  const bits = [];
  if (a.chain) bits.push(`chains to ${a.chain.count} more within ${fmtTiles(a.chain.radius)} at ${pct(a.chain.frac)}%`);
  if (a.resist) bits.push(`+${pct(a.resist.amount)}% ${a.resist.element} resistance`);
  if (a.allyScaling) bits.push(`+${pct(a.allyScaling.per)}%/ally within ${fmtTiles(a.allyScaling.range)} (max ${a.allyScaling.max})`);
  ...
  if (a.taunt && a.template !== TAUNT_TEMPLATE) bits.push(`draws ${a.taunt.max} enemies within ${fmtTiles(a.taunt.radius)} for ${a.taunt.duration}s`);
...
function statsLineBase(a) {
  switch (a.template) {
    // ROUND 190 (bug 1) -- the corpse templates had no line of their own, so
    // their rank effect read "· 12 mana".
    case 'corpseBlast':
      return `the nearest body within ${fmtTiles(a.range || 300)} bursts for ${a.base} damage plus 8% of its max health to all within ${fmtTiles(115)} · ${fmtCd(a.cooldown, a)}`;
    case 'corpseMiasma':
      return `the nearest body rots for 8s; enemies in its ${fmtTiles(130)} cloud take ${(a.dot && a.dot.dmgPerTick) || a.base} damage a tick · ${fmtCd(a.cooldown, a)}`;
    case 'corpseMine':
      return `primes up to 3 bodies in reach; each bursts for ${a.base} damage when an enemy comes within 2 tiles (30s) · ${fmtCd(a.cooldown, a)}`;
    case 'corpseDrain':
      return `every body in reach (up to 5) restores 7% of your ${a.resource === 'hp' ? 'health' : (a.resource || 'health')} · ${fmtCd(a.cooldown, a)}`;
    case 'raiseDead':
      return `the nearest body rises as a skeleton ally that fights for 30s, hitting for ${a.base} · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'projectileBall': {
      let s = `${a.base} dmg · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'barrierWall': {
      const what = { block: `a ${a.wallLength} wall nothing crosses`,
        ...
        || `a ${a.wallLength} wall`;
      return `${what} for ${a.wallDuration}s · ${fmtCd(a.cooldown, a)}`;
    }
    case 'reflectWard':
      if (a.reflectKind === 'debuff') {
        return `${Math.round(a.reflectChance * 100)}% chance to send an affliction back${a.buffDuration ? ` for ${a.buffDuration}s` : ''}${a.cooldown ? ` · ${fmtCd(a.cooldown, a)}` : ''}`;
      }
      return a.reflectKind === 'spell'
        ? `returns ${Math.round(a.reflectFrac * 100)}% of elemental damage taken${a.buffDuration ? ` for ${a.buffDuration}s` : ''}${a.cooldown ? ` · ${fmtCd(a.cooldown, a)}` : ''}`
    ...
    case 'breathCone': {
      let s = `${a.base} dmg to everything in a ${a.range} cone · ${fmtCd(a.cooldown, a)}`;
      if (a.dot) s += ` · +${a.dot.dmgPerTick}×${a.dot.ticks} ${a.dot.label}`;
      return s;
    }
    case 'volley': {
      let s = `${a.volleyCount} bolts, ${a.base} dmg each · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'aoeRing': {
      let s = `${a.base} dmg to all enemies within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown, a)}`;
      if (a.dot) s += ` · +${a.dot.dmgPerTick}×${a.dot.ticks} ${a.dot.label}`;
      return s;
    }
    // ---- ROUND 38 (sections 6.1-6.10) ----
    case 'bloomField': return `a field within ${fmtTiles(a.range)} restoring ${a.healPerSec} HP/s to you and allies for ${a.fieldDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'partyBuff': return `allies within ${fmtTiles(a.range)}: +${Math.round(a.partyDmgPct * 100)}% damage and +${a.partyPower} power for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'aoeHealPulse': return `restores ${a.healAmount} HP (+1% per 1% cast speed) to you and allies within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown, a)}`;
    case 'weakenRing': return `-${Math.round(a.sunder.amount * 100)}% armor, -${Math.round(a.slowPct * 100)}% speed to enemies within ${fmtTiles(a.range)} for ${a.sunder.duration}s · ${fmtCd(a.cooldown, a)}`;
    case 'rangeStrike': return `${a.base} dmg, up to ×${a.maxMult.toFixed(1)} at ${a.range} range · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'stackStrike': return `${a.base} dmg + ×${a.stackMult.toFixed(1)} of the target's remaining afflictions, consumed`
      ...
      + ` · ${fmtCd(a.cooldown, a)}`;
    case 'imbueStrike': return `weapon strike · applies ${a.dot.dmgPerTick}×${a.dot.ticks} ${a.dot.label}${a.sunder ? ` and -${Math.round(a.sunder.amount * 100)}% armor` : ''} · ${fmtCd(a.cooldown, a)}`;
    case 'thornsBuff': return `returns ${Math.round(a.thornsFrac * 100)}% of damage taken for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'townPortal': return `opens a portal to town — and back again · ${fmtCd(a.cooldown, a)}`;
    // ---- ROUND 49 ----
    ...
    case 'tauntPull': {
      const threat = (a.threatMult > 1)
        ...
        : '';
      return `draws up to ${a.tauntMax} enemies within ${fmtTiles(a.tauntRadius)} onto you for ${a.tauntDuration}s${threat} · ${fmtCd(a.cooldown, a)}`;
    }
    // ---- ROUND 49 ----
    case 'stealthVeil': {
      ...
      return `${who} veiled for ${a.stealthDuration}s: enemies notice you ${seen}% later${fast} · breaks on attack · ${fmtCd(a.cooldown, a)}`;
    }
    // ---- ROUND 48 ----
    case 'confuseTurn':
      return `turns up to ${a.maxTargets} enemies within ${fmtTiles(a.range)} against each other for ${a.confuseDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'fateReroll': {
      const what = {
    ...
    case 'triggeredPassive': {
      ...
      const what = e.kind === 'regenBurst' ? `${e.perSec} HP/s for ${e.duration}s`
        ...
        : e.kind === 'boltNearest' ? `${e.damage} lightning damage to the next nearest enemy within ${fmtTiles(e.range)}`
      ...
      return `${when}: ${what}${a.cooldown ? ` · ${fmtCd(a.cooldown, a)}` : ''}`;
    }
    case 'rangeBuff':
      return `x${a.rangeMult.toFixed(2)} attack and spell range for ${Math.round(a.buffDuration)}s · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'statBuff':
      return `+${Math.round(a.buffAmount * 100)}% ${statBuffLabel(a.buffStat)} for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'cooldownReset':
      return `clears the cooldown on ${a.resetCount === 1 ? 'one random ability' : `${a.resetCount} random abilities`} · ${fmtCd(a.cooldown, a)}`;
    case 'abilityLock':
      return `stops the target acting for ${a.lockSeconds}s within ${fmtTiles(a.range)} · ${fmtCd(a.cooldown, a)}`;
    case 'selfPower': return `+${Math.round((a.powerMult - 1) * 100)}% damage for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'selfCritBuff': return `+${Math.round(a.critChanceBonus * 100)}% crit chance for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'immunityBuff': return `immune to physical damage for ${a.immunityDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'timeFreeze': return `freezes every creature within ${fmtTiles(a.freezeRadius)} for ${a.freezeDuration}s · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'selfHeal': return `restores ${a.healAmount} HP ${healWho(a.healScope)} · ${fmtCd(a.cooldown, a)}`;
    case 'selfHot': return `${a.hotPerSec} HP/s for ${a.hotDuration}s ${healWho(a.healScope)} · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'cleanse':
      return (a.cleanseCount === Infinity
        ...
        + ` ${cleanseWho(a.healScope)} · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'resourceRestore':
      return (a.overTime
        ...
        : `restores ${a.restoreAmount} ${a.resource}`) + ` · ${fmtCd(a.cooldown, a)}`;
    ...
    case 'absorbShield': {
      ...
      return `${body}${src} · ${fmtCd(a.cooldown, a)}`;
    }
    case 'armorBuff': return `+${Math.round(a.armorBonus * 100)}% armor for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'sunderStrike': return `${a.base} dmg, -${Math.round(a.sunder.amount * 100)}% enemy armor for ${a.sunder.duration}s · ${fmtCd(a.cooldown, a)}`;
    ...
    // own separator.
    case 'chainStrike': return `${a.base} dmg · ${fmtCd(a.cooldown, a)}`;
    case 'dispelStrike':
      return `strips ${a.dispelCount || 1} effect${(a.dispelCount || 1) === 1 ? '' : 's'} from an enemy · ${fmtCd(a.cooldown, a)}`;
    case 'dash': return `${a.dashDist}px dash · ${fmtCd(a.cooldown, a)}`;
    case 'teleport': return `${a.teleportRange}px blink · ${fmtCd(a.cooldown, a)}`;
    case 'movementHaste': return `+${Math.round((a.speedMult - 1) * 100)}% move speed for ${a.buffDuration}s · ${fmtCd(a.cooldown, a)}`;
    case 'aura': {
      ...
      //
      // ROUND 174 -- AND IT IS NOT REPEATED IN THE RANK LINE.
      //
      // The user, on this card: "I think it's multiple effects just listed in
      // order but it seems to be referring to information or context not
      // available to the player." One of the things making it a list is that
      // the same sentence is on the card twice. The description already ends
      // with "Yours carries Unfailing Patience: what your field does to
      // something holds 4s after they leave it..." -- appended at `addKnown`,
      // see the note there -- and this line put the identical clause into the
      // Iron Rank Effect list beside it.
      //
      // The body keeps it and this drops it, because a signature is NOT a rank
      // effect: it does not change at iron, bronze or gold, and a line headed
      // "Iron Rank Effect" that lists something iron did not do is the card
      // telling the player something untrue as well as twice.
      // ROUND 190 (bug 1) -- the field's mechanics, not its flourish.
      if (row) {
        const clauses = auraEffectClauses(row.spec, row.trigger, row.effect);
        return clauses.length ? `${row.label}: ${clauses.join('; ')}`
          : `${row.label}: ${row.gained[row.gained.length - 1].adds}`;
      }
      ...
      if (a.auraEffect === 'brand') return `marks enemies within ${fmtTiles(a.auraRadius)} · +${Math.round((a.brandAmplify || 0) * 100)}% damage taken per stack · ${a.retaliate} back to anything that strikes inside it`;
      if (a.auraEffect === 'slow') return `-${Math.round(a.slowPct * 100)}% move speed to enemies within ${fmtTiles(a.auraRadius)}`;
      if (a.auraEffect === 'weaken') return `-${Math.round(a.sunderAmt * 100)}% armor on enemies within ${fmtTiles(a.auraRadius)}`;
      if (a.auraEffect === 'ward') return `+${Math.round(a.wardResist.amount * 100)}% ${a.wardResist.element} resistance within ${fmtTiles(a.auraRadius)}`;
    ...
    case 'perception': {
      ...
      // ROUND 190 (bug 1) -- the mechanics, not the flavour line. See
      // `senseEffectClauses`: every clause is a field the runtime reads.
      const clauses = senseEffectClauses(row);
      return clauses.length ? `${sense.label}: ${clauses.join('; ')}`
        : `${sense.label}` + (reach > 0 ? ` · +${reach}% pickup reach` : '');
    }
    case 'summonBonded': return `familiar strikes for ${a.familiarDmg} dmg every ${a.familiarInterval}s within ${fmtTiles(a.familiarRange)}`;
    case 'activeSummon': {
      const life = summonTimeShort(a.summonDuration);
      if (a.summonKind === 'trap') {
        return `${a.summonDmg} dmg blast, ${a.summonCharges} charges, armed ${life} · ${fmtCd(a.cooldown, a)}`;
      }
      ...
      if (a.summonRole === 'guard') {
        return `guardian: ${Math.round((a.summonHpPct || SUMMON_ROLES.guard.hpPct) * 100)}% of your health, `
          + `-${Math.round((a.summonGuardPct || 0) * 100)}% damage taken within `
          + `${a.summonGuardRadius}, taunts ${a.summonTauntMax} every ${a.summonTauntEvery}s, `
          + `lasts ${life} · ${fmtCd(a.cooldown, a)}`;
      }
      const what = a.summonKind === 'turret' ? 'turret' : 'creature';
      // ===== ROUND 204 (item 2.10.2, again) -- A ROLE THAT DEALS NOTHING ====
      //
      // Round 76 fixed this for the guardian ("a stats line that leads with a
      // zero teaches the player the ability is broken") and the same fault was
      // sitting on the other two roles with `dmgShare: 0`. Found in the
      // round-204 build review: a card reading "It never strikes." over a
      // stats line reading "3 dmg every 1.16s".
      //
      // The spec's `summonDmg` is the roll BEFORE the role's share is applied
      // -- `summonDamageFor` does that at spawn, against the player's live
      // rank -- so for these two roles the number printed here was never the
      // number the creature deals. It says what the creature is for instead.
      const roleDef = SUMMON_ROLES[a.summonRole || 'attack'];
      if (roleDef && roleDef.dmgShare === 0 && a.summonKind === 'creature') {
        return `${roleDef.label}: deals no damage, `
          + `${Math.round((a.summonHpPct || roleDef.hpPct || 0) * 100)}% of your health, `
          + `lasts ${life} · ${fmtCd(a.cooldown, a)}`;
      }
      // ROUND 204 (item 5) -- AND HOW MUCH LIFE IT HAS, which is the question
      // the user asked of this line: "how much health do the summons have? Do
      // they all have health to restore?" They do now, and a creature that can
      // be killed has to say so before the player sends it into something.
      //
      // As a PERCENTAGE of the caster's own maximum, because that is what the
      // number is -- `summonHpFor` scales off the player so a summon stays
      // relevant from iron to gold, and a flat figure printed here would be
      // true for one character and wrong for every other.
      const hp = a.summonKind === 'creature'
        ? `, ${Math.round((a.summonHpPct || (SUMMON_ROLES[a.summonRole || 'attack'] || {}).hpPct || 0) * 100)}% of your health` : '';
      return `${what}: ${a.summonDmg} dmg every ${a.summonInterval}s within ${fmtTiles(a.summonRange)}${hp}, lasts ${life} · ${fmtCd(a.cooldown, a)}`;
    }
    ...
    case 'weaponAffinity': {
      ...
      if (a.shotBounce) bits.push(`shots bounce to ${a.shotBounce} more`
        + ` within ${fmtTiles(a.shotBounceRange)}`);
      if (a.shotSpeedPct) bits.push(`+${Math.round(a.shotSpeedPct * 100)}% shot speed`);
      if (a.countsAs) bits.push(`a ${(WEAPONS[a.countsAs.from] || {}).name || a.countsAs.from} counts as a ${(WEAPONS[a.countsAs.as] || {}).name || a.countsAs.as} for abilities`);
    ...
    // ===== ROUND 204 (item 2.8.3) -- THE TRANSFORMATION'S OWN FIGURES ======
    //
    // The user: "Numbers now just says 40 stamina, which suggests the ability
    // costs 40 stamina but doesn't do anything at all."
    //
    // It did do something -- four things -- and every one of them lived in the
    // rank table rather than on the spec, so `statsLineFor` walked its list of
    // known fields, found none of them, and printed the cost alone. That is
    // also why round 202's rank diff could only say "4 of its figures grow":
    // there were no figures on the card for it to be talking about.
    //
    // Read at the spec's CURRENT rank, which `rankScaled` has already stamped
    // by the time the card asks -- the same route the aura and the perception
    // take for exactly this reason.
    case 'transform': {
      const st = transformAtRank(a.transform, a._transformRank || 'iron');
      if (!st) break;
      return [
        `+${st.attrAmount} ${ATTR_LABEL_LOCAL[st.attr] || st.attr}`,
        `+${Math.round(st.maxPoolPct * 100)}% max ${st.pool}`,
        `${st.regenPerSec} ${st.pool}/s`,
        `lasts ${st.duration}s`,
        fmtCd(a.cooldown, a),
      ].filter(Boolean).join(' · ');
    }
    ...
    // ROUND 176 -- THE FOURTH CAPABILITY HAD NO LINE AT ALL.
    //
    // Round 164 added `virulence` beside `waterWalk`, `twoHandWield` and
    // `unarmedFocus` in the capability atom and gave it no case here, so its
    // card came out blank: `test_round17` has reported "every generated
    // ability has a clean stats line — 20/400" every run since, and measured,
    // all of it is this one template emitting `stats: ""`.
    //
    // It is a permission check, so the line says what you may now do, in the
    // same six words `SUBTYPE_IMMUNE_TAGS` is written in. The name carries the
    // flavour ("Something In It That Should Not Take") and this states the
    // mechanic, which is the standing rule for every card in the game.
    case 'virulence': {
      if (a.piercesAll) return 'your afflictions land on anything, whatever it is made of';
      const tags = (a.piercesTags || []);
      if (!tags.length) return 'your afflictions ignore an immunity';
      const label = { blood: 'bleeds', wounding: 'wounds', poison: 'poisons',
        curse: 'curses', burning: 'burns', holy: 'holy afflictions',
        unholy: 'unholy afflictions' };
      const named = [...new Set(tags.map(t => label[t] || t))];
      const list = named.length > 1
        ? `${named.slice(0, -1).join(', ')} and ${named[named.length - 1]}`
        : named[0];
      return `your ${list} land on creatures that are immune to them`;
    }
...
function swiftClause(spec) {
  ...
  const cd = `it comes back in ${fmtCd(spec.cooldown, spec).replace(/\s*cd$/, '')}`;
 ...
 */
/**
 * ROUND 190 (item 6.3) -- THE SPAMMABLE TIER.
 *
 * An attack you can press again the moment it lands: its cooldown drops under
 * SPAM_CD_MAX, its damage is priced down to match (a hit you can repeat six
 * times in the time a 6s-cooldown spell repeats once cannot hit like it), and
 * its cost is low -- but the cost climbs with the streak exactly as the damage
 * does, so a long streak is a decision about your pool rather than free.
 * Higher ranks answer the cost: bronze halves the climb, silver lets health
 * pay when the pool runs dry, gold refunds half the cost on a kill (see
 * rankAspectsFor).
 */
export const SPAM_CD_MAX = 1.2;
export const SPAMMABLE_TEMPLATES = ['projectileBall', 'volley', 'chainStrike', 'breathCone',
  'sunderStrike', 'rangeStrike', 'aoeRing', 'stackStrike'];
export function makeSpammable(spec, roll = null) {
  const r = (k, n) => (roll ? roll(k, n) : 0);
  const oldCd = typeof spec.cooldown === 'number' && spec.cooldown > 0 ? spec.cooldown : SPAM_CD_MAX;
  const cd = Math.min(oldCd, Math.round((0.6 + r('spamcd', 7) * 0.1) * 10) / 10);
  if (typeof spec.base === 'number' && oldCd > cd) {
    // Damage per second is held roughly level, then shaded down: spam trades
    // burst for rhythm, it does not win on both.
    spec.base = Math.max(3, Math.round(spec.base * Math.max(0.4, Math.pow(cd / oldCd, 0.55))));
  }
  spec.cooldown = cd;
  spec.spammable = true;
  // Pressed again at once: nothing to wind up, and the streak IS its condition.
  spec.castTime = 0;
  // A rider that summons or buffs per cast would fire every 0.6s.
  delete spec.escort; delete spec.hasteOnUse; delete spec.blinkOnUse;
  if (spec.scaleOn !== 'successiveUses') {
    spec.scaleOn = 'successiveUses';
    spec.scalePer = spec.scalePer || 0.06;
    spec.scaleCap = spec.scaleCap || 0.30;
    spec.scaleVariant = spec.scaleVariant || 0;
    delete spec.scaleDelay;
  }
  spec.spamCostPer = Math.round((0.15 + r('spamcost', 6) * 0.02) * 100) / 100;
  return spec;
}
...
export function applyEssenceFlavour(spec, essDef, stone, cat, roll0, comboBase, opts = {}) {
  ...
  // ===== ROUND 203 (item 5) -- AND, VERY RARELY, TRANSCENDENT =============
  //
  // This is the ONE door a generated ability's element comes through (round
  // 53's note above), so it is the one door transcendent comes through too.
  // Everything about the roll is deliberately narrow:
  //
  //   * ONLY AN ABILITY THAT DEALS DAMAGE. A transcendent heal is not a thing
  //     the books describe and would be a type on a spec with nothing to type.
  //   * ONE IN SEVERAL HUNDRED, by the kit's rank (TRANSCENDENT_ODDS), which
  //     is rarer at every rank than the game's own one-in-forty rare seat --
  //     "Even at Silver and Gold transcendent damage is rare".
  //   * ALWAYS GATED below silver, and the gate is chosen from the six the
  //     user's "narrow conditions or significant setup" describes. The gate is
  //     what keeps "ignores rank disparity" from being a hole in round 43's
  //     tyranny of rank -- see transcendent.js.
  //   * THE ORDINARY ELEMENT IS KEPT as `fallbackElement`, so on every swing
  //     the gate is shut the ability still looks, sounds and resists like the
  //     essence it came from rather than becoming untyped.
  //
  // Seeded off the kit and the socket, never off a counter, so the same kit
  // always produces the same answer.
  if (TRANSCENDENT_ODDS && (spec.category === 'attack' || typeof spec.base === 'number')) {
    const rank = opts.kitRank || 'iron';
    const odds = TRANSCENDENT_ODDS[rank] || 0;
    const h = stableHash(`${essDef && essDef.id}|${cat.key}|${opts.stoneId || ''}|transcendent`);
    if (odds > 0 && (h % 100000) / 100000 < odds) {
      spec.fallbackElement = mat.element;
      spec.element = TRANSCENDENT;
      if (RANK_ORDER.indexOf(rank) < RANK_ORDER.indexOf('silver')) {
        spec.transcendentGate = TRANSCENDENT_GATE_KEYS[
          stableHash(`${essDef && essDef.id}|${cat.key}|gate`) % TRANSCENDENT_GATE_KEYS.length];
      }
    }
  }
  // ===== ROUND 203 (items 2, 3, 4) -- THE THREE SHAPES ROUND 201 MADE =====
  // SAYABLE AND NOTHING SAID.
  //
  // Each is rolled at the same one door the element comes through, each is
  // rare, and each is only offered to a spec that can carry it. The point of
  // all three is the same: round 201 proved the CARD could print them by
  // writing ten canon abilities out by hand, and a shape only a fixture can
  // reach is a shape the game does not have.
  {
    const tRoll = (salt) => (stableHash(`${essDef && essDef.id}|${cat.key}|${opts.stoneId || ''}|${salt}`) % 10000) / 10000;

    // --- item 2: a cooldown computed at use, and one measured in hours -----
    //
    // [Blessing of Readiness] is the canon shape: its cooldown is the time it
    // just removed from someone else's. Offered only to an ability that HAS
    // something to measure -- one that reduces a cooldown or grants a timed
    // effect -- because "Varies" on an ability with nothing varying is a word
    // where a number should be.
    if (spec.kind === 'active' && typeof spec.cooldown === 'number') {
      // WHAT COUNTS AS "something to measure" WAS WRONG IN THE FIRST CUT, and
      // the round-203 suite found it by asking the only question that matters
      // of a rule table: is every rule in it reachable? `cooldownReduction` is
      // a PASSIVE field (`cooldownPassive` folds it into minorStats); no
      // active carries it, so `reciprocal` -- the canon shape, [Blessing of
      // Readiness], the one the whole rule was written for -- could never be
      // stamped and every Varies cooldown in the game was a durationOfGrant.
      //
      // The abilities that actually hand time back are the ones that SAY so:
      // the `cooldownReset` template, and the `refresh` rider that shaves a
      // couple of seconds off everything. Those are what a reciprocal cooldown
      // is measured against, so those are what may carry it.
      const givesTime = spec.template === 'cooldownReset'
        || spec.cooldownReduction > 0
        || (spec.composedRiders || []).includes('refresh');
      if ((givesTime || spec.buffDuration > 0) && tRoll('varies') < 0.06) {
        spec.cooldownFloor = Math.max(3, Math.round(spec.cooldown * 0.25));
        spec.variesCooldown = givesTime ? 'reciprocal' : 'durationOfGrant';
        // ROUND 203 -- THE FLAG IS THE FLAG; THE FIELD STAYS A NUMBER.
        //
        // The first cut put the string 'varies' into `spec.cooldown`, and the
        // regression found what that costs within the hour: round 49's taunt
        // band read `cooldown > 0` and rejected two well-formed abilities,
        // round 27's armour band did the same, and the HUD's cooldown ring
        // divides by it. A sentinel in a field forty-nine call sites do
        // arithmetic on is a promise that every one of them will be checked,
        // which is not a promise anybody can keep.
        //
        // So `cooldown` holds the FLOOR -- a true statement, the shortest this
        // ability can ever cool -- and `variesCooldown` is the flag that makes
        // the card say "Varies." and the runtime compute the real figure at
        // use. Every numeric reader keeps working and is not lied to.
        spec.cooldown = spec.cooldownFloor;
      } else if (spec.cooldown >= 45 && tRoll('hours') < 0.05) {
        // An already-long cooldown becomes a canon one. Only ever the long
        // ones: a six-second ability on an hours-scale cooldown is not a
        // stronger ability, it is a different one nobody asked for.
        withCanonCooldown(spec, [1, 6][stableHash(`${cat.key}|hrs`) % 2]);
      }
    }

    // --- item 3: a per-second drain of two pools --------------------------
    //
    // [Eternal Moment]'s shape. Offered only to a CHANNELLED ability -- one
    // that holds something open for a duration -- because a drain per second
    // on an instant is a drain that ticks once.
    // The channelled fields, measured rather than guessed: a census of 80 kits
    // found `duration` on 12 actives and `buffDuration` on 157, so keying on
    // `duration` alone made this one ability in eight thousand -- a shape the
    // game technically had and no player would ever meet.
    const CHANNELLED = ['duration', 'freezeDuration', 'buffDuration', 'shieldDuration',
      'fieldDuration', 'wallDuration', 'stealthDuration'];
    if (spec.kind === 'active' && CHANNELLED.some(f => spec[f] > 0)
        && tRoll('drain') < 0.05) {
      const per = 6 + (stableHash(`${cat.key}|drain`) % 6);
      spec.drain = [{ type: 'mana', perSec: per }, { type: 'stamina', perSec: per }];
      // The entry price comes down: you are paying for every second of it.
      if (spec.cost && spec.cost.amount > 4) spec.cost = { ...spec.cost, amount: Math.round(spec.cost.amount * 0.4) };
    }

    // --- item 4: an ally, and never yourself -------------------------------
    //
    // [Blessing of Readiness]: "Can only be used on others, not yourself."
    // Offered only where there is somebody else for it to land on -- a spec
    // that already names a heal or buff scope -- and it makes the ability
    // STRONGER in exchange, because a boon you cannot use on yourself is
    // strictly worse than the same boon unrestricted.
    if (spec.healScope || spec.buffScope) {
      if (tRoll('allynotself') < 0.08) {
        if (spec.healScope) spec.healScope = 'allyNotSelf';
        if (spec.buffScope) spec.buffScope = 'allyNotSelf';
        for (const f of ['healAmount', 'tickAmount', 'buffAmount']) {
          if (typeof spec[f] === 'number') spec[f] = Math.round(spec[f] * 1.35);
        }
      }
    }
  }
  ...
  // ROUND 174 -- THE STONE PULLS ITS OWN LEVER FIRST, WHEN IT CAN PULL ONE.
  //
  // The user: "A wolf stone of the cage gave me a movement speed ability.
  // While that is somewhat wolf themed, it's the opposite of what a cage stone
  // should be giving you. A wolf themed stun, or spectral wolves that surround
  // an enemy holding them in place."
  //
  // The running order below is the ESSENCE's, and until this round it was the
  // only one -- so the twist an ability carries, the sentence a player reads
  // and the scalars it actually applies all came from the essence and the
  // stone contributed a colour and a word. That is the second half of the same
  // fault FAMILY_LEVER fixes in the composer: the essence is the noun and the
  // stone was not the verb, it was an adjective on the label.
  //
  // `leverApplies` is the gate that already exists, and it is the right one:
  // a stone whose lever has nothing to grab on this template hands off to the
  // essence exactly as a declining essence lever does, rather than asserting a
  // twist it did not make. So a Cage stone reaches for `muzzle` -- "takes the
  // tempo and the answer away from what it faces" -- and gets it wherever the
  // template can carry it.
  //
  // TWO TIMES IN THREE, NOT ALWAYS -- and the third time is not a hedge, it is
  // a floor this round broke and had to put back. Letting the stone win
  // outright took `test_round52_hot` from 35/35 to 31/35: its rule is that a
  // POOL offering an instant heal offers an over-time heal too, and a pool is
  // twelve candidates from one socket, so a stone that answers for every one
  // of them can starve a lever the ESSENCE is obliged to express. Seeded off
  // the category like every other roll here, so a given ability resolves the
  // same way on every rebuild.
  //
  // AND THE STONE MAY NOT BREAK THE ESSENCE'S CHARTER. This is the rule that
  // took the round-52 suite from 31/35 back to whole, and it is the right rule
  // rather than a tuning: Renewal's charter REFUSES affliction -- round 52's
  // own line is "renew may produce nothing that harms" -- and it enforces that
  // by not carrying the `linger` lever. A stone reaching past the motif
  // reached past that refusal too, and a Serpent stone put a festering wound
  // on a healing essence. So the stone's lever is admitted only when nothing
  // it brings is something this essence has forsworn. The stone is the verb;
  // it is not a way round what the essence is.
  const stoneLeverRaw = stone && stone.family ? FAMILY_LEVER[stone.family] : null;
  const essCharter = charterFor(motif.levers || []);
  const stoneCharter = stoneLeverRaw ? LEVER_CHARTERS[stoneLeverRaw] : null;
  //
  // IT MAY NOT BRING HARM TO AN ESSENCE THAT HAS NONE.
  //
  // Two wrong versions preceded this one and both are worth the lines.
  //
  // The first tested the essence's `mayNot` and never fired once -- the lever
  // charters express themselves almost entirely through `may`, and
  // `charterFor` builds `deny` from `mayNot`, which is empty for every lever
  // this rule touches. A check that cannot fire is decoration, and the
  // round-52 suite stayed at 32/35 and said so.
  //
  // The second demanded the stone's whole `may` set be a SUBSET of the
  // essence's, which is sound and far too wide: it fixed the suite and took
  // the stone's share of twists from 30.6% down to 13.0%, refusing a Swift
  // stone on a healing essence because `movement` is not something Renewal
  // does. Nothing was being protected there. A healer who moves faster is not
  // a contradiction; a healer who leaves a festering wound is.
  //
  // So the rule is exactly the width of round 52's own two sentences -- "renew
  // may produce nothing that harms" and "a linger essence still rots but never
  // mends" -- and no wider. A stone may bring anything its essence already
  // allows, and anything off those two axes besides; what it may not do is
  // reverse the essence's POLARITY, in either direction.
  const stoneLever = (stoneLeverRaw && stoneCharter && essCharter && !essCharter.empty
    && !(stoneCharter.may || []).some(f => POLARITY_FAMILIES.has(f) && !essCharter.allow.has(f)))
    ? stoneLeverRaw : null;
  const stoneLeads = !!stoneLever && roll('stonelever', STONE_LEVER_ODDS[1]) < STONE_LEVER_ODDS[0];
  const first = (stoneLeads && leverApplies(stoneLever, spec, tags0))
    ? stoneLever
    : leverForCategory(essDef, cat, roll, opts.spine);
  ...
  switch (lever) {
    ...
    case 'allies': {
      ...
      // at 6.4% of every ability.
      {
        ...
        const bank = [
          () => `Its effect increases by ${per}% for each ally within ${fmtTiles(rng)}, up to ${max} allies${tail}.`,
          () => `Every ally inside ${rng} makes it ${per}% stronger, to a limit of ${max}${tail}.`,
          () => `It draws ${per}% more from each of up to ${max} companions standing within ${fmtTiles(rng)}${tail}.`,
          () => `Fighting beside others pays: +${per}% per ally within ${fmtTiles(rng)}, capped at ${max}${tail}.`,
        ];
    ...
    case 'chain': {
      ...
      mech = `It then leaps to ${spec.chain.count} more ${spec.chain.count === 1 ? 'enemy' : 'enemies'} within ${fmtTiles(spec.chain.radius)}, each hit dealing ${pct(spec.chain.frac)}% of the original damage.`;
      break;
    }
    ...
    case 'renew': {
      {
        ...
        if (spec.template === 'selfHot') {
          ...
          // ROUND 202 -- "after it should have stopped" is the same shape as
          // "than it otherwise would": it measures against a duration the
          // player was never told. The duration itself is on the card.
          mech = `The ${mat.hot ? String(mat.hot).toLowerCase() : 'mending'} keeps working for a further ${add}s.`;
          ...
          // ROUND 202 -- was "restores N% more than it otherwise would", the
          // imaginary-baseline construction verbatim. Both halves the rider
          // actually changes are numbers, so it prints them.
          mech = `It restores ${spec.tickAmount} a tick and reaches ${spec.auraRadius}.`;
    ...
    case 'burst': {
      ...
      // three sentences that described the imaginary cost are gone with it.
      {
        ...
        const cdText = fmtCd(spec.cooldown, spec).replace(/\s*cd$/, '');
        const bank = [
    ...
    case 'anchor': {
      ...
      {
        const sp = pct(spec.bindOnHit.slowPct), du = spec.bindOnHit.duration;
        const bank = [
          () => `Whatever it hits is slowed by ${sp}% for ${du}s.`,
          () => `Anything struck moves ${sp}% slower for ${du}s.`,
          () => `A hit drags at the target: ${sp}% slower for ${du}s.`,
        ];
        mech = bank[phraseIndex(spec, mat, bank.length)]();
      }
      break;
    ...
    case 'call': {
      if (typeof spec.familiarDmg === 'number') {
        ...
        // ROUND 202 -- was "arrives stronger than the summoning would normally
        // allow", which compares the summon to a norm the player has never
        // been shown. It carries the actual figure now: the rider multiplies
        // `familiarDmg` by 1.3 and that is a number worth printing.
        mech = `What it summons hits for ${spec.familiarDmg}.`;
      } else {
    ...
    case 'taunt': {
      if (spec.template === TAUNT_TEMPLATE) {
        ...
        mech = `Using it also pulls ${spec.taunt.max} enemies within ${fmtTiles(spec.taunt.radius)} onto you for ${spec.taunt.duration}s.`;
      }
      break;
    }
    default: break;
  ...
  // ===== ROUND 204 (items 2.3.2, 2.5.1) -- AND IT WAS NOT ONLY FIRING THEN ==
  //
  // The user, on two spear strikes that both grew an armour self-buff: "In no
  // way does a spear strike make sense to grant an armor buff. It's clearly
  // the generator seeing the word armor and just tacking that on."
  //
  // The conclusion is right and the mechanism is not: names are chosen AFTER
  // the mechanic (round 48), so nothing here has ever read the name. What
  // actually happened is this block, and it went wrong in two separate ways.
  //
  // FIRST, the guard was on the wrong variable. It read `!mech` -- "no clause
  // was produced" -- while the comment above it describes "every authored
  // lever has genuinely declined", which is `!lever`. Those are different
  // things, because `raw` -- the commonest lever on a weapon build -- applies
  // successfully and deliberately returns no clause (see its `break`, and the
  // note there calling the empty string "a supported state, not an omission").
  // So every raw-lever strike in the game pulled its lever AND took the
  // fallback, which is how a spear charge ends up hardening your armour.
  //
  // SECOND, even when every lever has declined, armour is the wrong thing to
  // hand an ABILITY THAT HITS THINGS. The fallback exists so the essence
  // contributes something rather than nothing; contributing the wrong thing is
  // worse than contributing nothing, and "nothing" is already a supported
  // outcome here. So it is offered only where a defensive clause belongs: a
  // defensive spec, or a passive, which is a standing property of the bearer.
  if (!mech && !lever) {
    const fits = isDefensive(spec) || spec.kind === 'passive';
    if (fits) {
    ...
    // ...and when it does NOT fit, nothing happens: no clause, and no
    // `_leverFolded` either. The first cut set the flag here, which is wrong
    // in a way test_round55 caught within the round -- `_leverFolded` means
    // "a lever folded into the base line, so rebuild that line", and
    // rebuilding it calls `mechanicalDesc`, which has no case for
    // summonWeapon, summonArmor, barrierWall, virulence or elementPierce and
    // returns undefined for them. 77 abilities in 12,000 had their whole
    // description replaced with the string "undefined".
  }
  ...
  if (sig && SCALING_SIGNATURES.has(sig)) {
    ...
    let mode = scaleModeForFamily(fam);
    const field = scalableMagnitude(spec);
    // ===== ROUND 190 (item 6.2 / 6.3) -- A STREAK NEEDS A SHORT COOLDOWN ====
    //
    //   "The ability gives a bonus for consecutive use but has a 19 second
    //    cooldown meaning it can't be used consecutively at all."
    //   "Some special attacks or spells should have effectively no cooldown and
    //    be 'Spammable' low damage attacks ... if it rolls increased damage with
    //    consecutive uses it also rolls increased cost with multiple uses."
    //
    // A streak lapses after SCALE_STREAK_WINDOW seconds, so it is only a real
    // condition on something that can be pressed again inside that window.
    // Two outcomes, never the old third:
    //   * an ESCALATING attack (the `escalation` lever -- Force, which is what
    //     escalation means) becomes SPAMMABLE: near-zero cooldown, low damage
    //     and cost, and a cost that climbs with the streak just as the damage
    //     does (see spamCostPer, WorldScene._payAbilityCost);
    //   * anything else that rolled the streak scales on your own hurt
    //     instead -- a condition that can actually come true for it.
    if (mode === 'successiveUses') {
      const hurts = typeof spec.base === 'number' && spec.base > 0 && spec.kind === 'active'
        && SPAMMABLE_TEMPLATES.includes(spec.template);
      if (hurts && sig === 'escalation') makeSpammable(spec, roll);
      else mode = 'selfDepletion';
    }
    if (mode && field) {
  ...
  // ===== ROUND 204 -- AND THE BASE LINE IS BUILT EVEN WITH NO CLAUSE =====
  //
  // `!mech && _leverFolded` was the only route to building the base line
  // without a lever clause, and round 204 created a second case: a lever
  // applied, produced no clause, and the armour fallback no longer fires
  // because the ability is an offensive one. Several templates -- summonWeapon,
  // summonArmor, barrierWall -- never set `desc` in the template switch at all
  // and have always relied on this line for it, so leaving them alone gave 77
  // abilities in 12,000 a description of `undefined`. test_round55 has checked
  // for exactly that string since round 55 and caught it inside the round.
  //
  // So the condition is what it always meant: no clause, rebuild the base.
  if (!opts.keepText && !mech) {
    const rebuilt = mechanicalDesc(spec, mat);
    // ...and only if there is one. A template with no case in `mechanicalDesc`
    // keeps whatever the switch gave it, which is the strictly better of the
    // two wrong answers available.
    if (rebuilt) spec.desc = rebuilt;
...
// ROUND 202 -- `magnitudeVerb` and `magnitudePhrase` DELETED.
//
// They turned a lever's roll multiplier into "It hits 24% harder" -- literally
// the thing the user described this round: "the generator is keeping track of
// a baseline ability and then if it rolls better in some way it's trying to
// state that". Round 133 deleted the call site and left the functions, so for
// sixty-nine rounds the sentence sat here able to come back the moment anyone
// wanted a magnitude phrase. Deleted rather than left dead, because dead code
// that produces exactly the fault a round was spent removing is a loaded gun.
...
const GENERIC_SIGNATURE_FRAMES = (() => {
  ...
  // ROUND 202 -- THE CONFLUENCE BANK WAS NEVER COUNTED, and it feeds the same
  // `spec.desc = entry.desc` path. It is one row per kit rather than twenty, so
  // it leaked rather than flooded -- but "A shoulder that reaches further and
  // hits heavier than it should" and "A limb that does not belong to you
  // strikes from further than it should" shipped verbatim, and both are the
  // compares-itself-to-an-unstated-expectation shape this round is removing.
  // Counted here so the same >=4 rule applies to it; the ones that are merely
  // rare are caught by the filler guard instead.
  for (const name of MARQUEE_CONFLUENCES) {
    for (const e of (confluenceSignaturesFor(name) || [])) {
      const f = frameOf(e.desc); if (f) n[f] = (n[f] || 0) + 1;
    }
  }
  return { frames: new Set(Object.keys(n).filter(k => n[k] >= 4)), frameOf };
})();
function isFilledTemplateDesc(desc) {
  if (GENERIC_SIGNATURE_FRAMES.frames.has(GENERIC_SIGNATURE_FRAMES.frameOf(desc))) return true;
  // ROUND 202 -- and a signature that is filler is not authored however rare
  // its frame is. The frame counter catches a sentence used twenty times; this
  // catches the one used once that should never have been written.
  return !!fillerReason(desc);
}
...
function mechanicalDesc(spec, mat) {
  ...
  switch (spec.template) {
    ...
    case 'projectileBall': {
      // ROUND 190 -- one stock line was 5% of every ability in the game; the
      // same mechanic now reads one of three ways, chosen by the name so a
      // given ability always says the same thing.
      const h = [...String(spec.name || spec.id || '')].reduce((n, c) => (n * 31 + c.charCodeAt(0)) >>> 0, 7);
      return [
        `Throws a bolt of ${el} that deals ${spec.base} damage to the first enemy it hits.`,
        `Fires a single shot of ${el} that deals ${spec.base} damage to whatever it strikes first.`,
        `Casts a bolt of ${el} ahead of you; the first enemy in its path takes ${spec.base} damage.`,
      ][h % 3];
    }
    case 'aoeRing':
      return `Bursts outward, dealing ${spec.base} damage to every enemy within ${fmtTiles(spec.range)}.`;
    case 'breathCone':
    ...
    case 'elementPierce':
      // ROUND 215 -- "can no longer be resisted" at IRON, which is the first
      // tier this ability has. There is no earlier state in which it was
      // resisted. See the note on IMPROVEMENT_TELLS in rankLadders.js: the
      // guard found this one after the ladder rungs were fixed, in a
      // completely different file, which is the whole reason it was written
      // as a property rather than a list of sentences.
      return `Your ${spec.pierceElement || el} damage ignores resistance. Enemies that would shrug it off take it in full.`;
    case 'aoeHealPulse':
      ...
      return `Restores ${spec.healAmount} health to you and every ally within ${fmtTiles(spec.range)}, and more the faster you cast.`;
    case 'partyBuff':
      ...
      return `Every ally within ${fmtTiles(spec.range)} strikes ${pct(spec.partyDmgPct)}% harder for ${secs(spec.buffDuration)}, and fights with ${spec.partyPower} more power while it holds.`;
    case 'bloomField':
    ...
    case 'weakenRing':
      return `Strips ${pct(spec.sunder.amount)}% armour and ${pct(spec.slowPct)}% movement speed from every enemy within ${fmtTiles(spec.range)}.`;
    case 'rangeStrike':
    ...
    case 'abilityLock':
      return `Silences one enemy within ${fmtTiles(spec.range)}, taking its next action away for ${secs(spec.lockSeconds)} and putting out whatever it had standing.`;
    case 'timeFreeze':
      ...
      return `Holds every creature within ${fmtTiles(spec.freezeRadius)} in place for ${secs(spec.freezeDuration || 5)}.`;
    case 'confuseTurn':
      return `Turns up to ${spec.confuseMax || 2} enemies within ${fmtTiles(spec.range)} against each other for ${secs(spec.confuseDuration || 6)}.`;
    case 'dash':
    ...
    case 'aura':
      ...
      switch (spec.auraEffect) {
        ...
        case 'slow':
          return `A standing field that slows every enemy within ${fmtTiles(spec.auraRadius)} while they stay in it.`;
        case 'weaken':
          return `A standing field that wears down the armour of every enemy within ${fmtTiles(spec.auraRadius)}.`;
        case 'ward':
          return `A standing field that grants ${pct(spec.wardResist.amount)}% ${spec.wardResist.element} resistance to you and any ally within ${fmtTiles(spec.auraRadius)}.`;
        ...
        default:
          return `A standing field of ${el}: everything within ${fmtTiles(spec.auraRadius)} is marked, taking ${pct(spec.brandAmplify || 0)}% more damage per mark, and anything that strikes inside it takes ${spec.retaliate} back.`;
      }
    ...
    case 'triggeredPassive': {
      ...
      const what = {
        ...
        boltNearest: `a bolt leaps to the nearest enemy within ${fmtTiles(e.range)}, dealing ${e.damage} damage`,
    ...
    case 'activeSummon': {
      ...
      if (spec.summonKind === 'trap') {
        ...
        return `${place} ${noun}${where}. It waits ${life}, and the first ${spec.summonCharges} enemies to come within ${fmtTiles(spec.summonRange)} take ${spec.summonDmg} damage in a blast around it.`;
      }
      if (spec.summonKind === 'turret') {
        return `Sets down ${noun} for ${life}. It cannot move, and it strikes anything within ${fmtTiles(spec.summonRange)} for ${spec.summonDmg} damage every ${spec.summonInterval}s.`;
      }
    ...
    case 'tauntPull':
      return `A challenge that pulls up to ${spec.tauntMax} enemies within ${fmtTiles(spec.tauntRadius)} onto you for ${secs(spec.tauntDuration)}.`;
    case 'stealthVeil':
...
export function specTags(spec) {
  ...
  return {
    ...
    // ROUND 204 (item 2.2.1) -- MANY AT ONCE, which is what a volley is.
    // The user: "Spear Volley ability specifically notes it's a single shot. A
    // volley should be at grouping. Just a bad name vs effect."
    //
    // Rule 2 of NAME_REQUIRES already gated `volley` on `projectile`, and a
    // single bolt satisfies that -- the rule asked whether anything was
    // thrown, not how much. This is the missing half.
    many: spec.template === 'volley' || spec.template === 'breathCone'
      || spec.template === 'aoeRing' || spec.template === 'chainStrike'
      || !!spec.isAoe || (spec.projectiles > 1) || (spec.chainCount > 0)
      || (spec.explodeRadius > 0),
    // ROUND 204 (item 2.11.1) -- and whether it is a SPELL at all. "Linked
    // Spell" arrived as a special attack that required an axe in hand.
    casts: !spec.requiresWeapon
      && !(spec.cost && spec.cost.type === 'stamina' && spec.category === 'attack'),
    // ROUND 205 (item 3.2) -- whether anything about this ability's SOURCE
    // curses. The user: "an awakening stone of the monkey is probably more
    // about strength/speed than curses." Read off the family both catalogues
    // already carry, so the list is families rather than a hand-kept roster of
    // stones that would drift the first time one was added -- this project's
    // fault class 3. See CURSE_FAMILIES.
    curses: curseSourced(spec),
    ...
    // ROUND 207 -- the CREATURE this ability bonds, when it bonds one. Round
    // 204 reported "Call the Bee summons a lizard" and it has been unfixed
    // for three rounds because nothing the name rules could read knew what
    // creature was on the other end. See namePromisesWrongBeast.
    familiar: spec.familiarFamily || null,
  };
}

// ===========================================================================
// ROUND 205 (item 3.2) -- WHO IS ALLOWED TO CURSE.
//
// The user, on a Monkey stone producing a "Wounding Curse": "Maybe a scorpion,
// spider, or snake, but an awakening stone of the monkey is probably more
// about strength/speed than curses."
//
// `serpent` is the family that holds Snake, Spider, Lizard and Venom -- the
// three the user named, and the reason this is a family test rather than a
// list of three stones. The other three are the families whose whole idea is
// the harm that keeps working after the blow: `dark` (Sin, Malign, Void),
// `death` (Blight, Corrupt, Undeath, the Last Door) and `blood` (Hunger,
// Feast, Flesh). Everything else -- the beasts, the tools, the weather, the
// weapons, the orders -- may hit you very hard and may not curse you.
//
// Both catalogues use the same `family` vocabulary, so one set answers for the
// stone and for the essence, and EITHER may justify the word: a Venom essence
// delivered through an Iron stone is still a poisoner.
// ===========================================================================
export const CURSE_FAMILIES = new Set(['serpent', 'dark', 'death', 'blood']);

function curseSourced(spec) {
  if (!spec) return false;
  const st = STONE_THEMES[spec.stoneId];
  if (st && CURSE_FAMILIES.has(st.family)) return true;
  const es = ESSENCE_CATALOG[spec.essenceId];
  if (es && CURSE_FAMILIES.has(es.family)) return true;
  return false;
}

// A name that says one of these has to be describing a spec that does it.
/** "a, b and c". English, not a join -- the same shape line 6166 builds inline
 *  for summon roles, named once now that a second caller needs it. */
...
/** "a spear", "an axe". English, not a guess. */
function anArticle(word) {
  const w = String(word || '').trim();
  return `${/^[aeiou]/i.test(w) ? 'an' : 'a'} ${w}`;
}

// ROUND 205 (item 3) -- the curse vocabulary, used by the two NAME_REQUIRES
// rules at the bottom of the table. Named once rather than written twice: two
// rules share it, and a word added to one and not the other would be exactly
// the "a lexical guard catches the phrasing reported and misses the one
// written next" fault round 204c was spent on.
const NAME_CURSE_RE = /\b(curse|cursed|curses|hex|hexed|jinx\w*|malediction|malefic\w*|anathema)\b/i;
...
const NAME_REQUIRES = [
  ...
  // ROUND 204 -- `volley` and `barrage` moved OUT of this rule and into the
  // `many` rule below. They were failing here on the `volley` template itself,
  // because `projectile` means the single-bolt template specifically; a rule
  // that rejected "Volley" on a volley was the reverse of the one the user
  // asked for.
  { re: /\b(bolt|shot|arrow|ray|lance|missile|javelin|dart|quarrel)\b/i, need: 'projectile' },
  ...
  // ROUND 204 (item 2.2.1) -- a word that promises MORE THAN ONE. Split out of
  // rule 2 rather than added to it: "bolt" and "arrow" are honest names for a
  // single missile and must stay legal on one, while "volley", "hail" and
  // "barrage" are claims about number.
  { re: /\b(volley|barrage|hail|salvo|storm of|rain of|swarm|fusillade|broadside)\b/i, need: 'many' },
  // ROUND 204 (item 2.11.1) -- and a word that promises a SPELL. "Linked
  // Spell" was a special attack that wanted an axe in your hands, which is the
  // one thing a spell is not.
  { re: /\b(spell|incantation|cantrip|invocation|sorcery|thaumaturgy)\b/i, need: 'casts' },
  // ===== ROUND 205 (item 3) -- A CURSE IS CAST, AND BY SOMETHING THAT CURSES
  //
  // The user, on [Wounding Curse]:
  //
  //   3.1 "Curse reads as a spell, but its a special attack."
  //   3.2 "I also struggle to see the connection to a awakening stone of the
  //        monkey. Maybe a scorpion, spider, or snake, but an awakening stone
  //        of the monkey is probably more about strength/speed than curses."
  //
  // Two claims in one word, so two rules against one vocabulary. `casts` is
  // round 204's tag and answers 3.1: a curse that wants an axe in your hands
  // and spends stamina is a swing, whatever it is called. `curses` is new and
  // answers 3.2: the word is a claim about WHERE the ability comes from, and
  // a Monkey is clever fingers.
  //
  // Both, not either. "Wounding Curse" off a Snake essence delivered by a
  // spear is still a swing; "Curse of the Clever Fingers" cast as a spell is
  // still a monkey.
  { re: NAME_CURSE_RE, need: 'casts' },
  { re: NAME_CURSE_RE, need: 'curses' },
];
...
export function nameContradictsSpec(name, tags, identityWords) {
  ...
  // ===== ROUND 205 (item 2.1) -- ON WORD BOUNDARIES ======================
  //
  // This was a bare substring replace, and it was quietly disarming the whole
  // rejection test. An Awakening Stone of the RAT strips "rat" -- and
  // "Resto{rat}ive" contains it, so "Restorative Hibernation" came out of here
  // as "Resto ive Hibernation", which no rule below can recognise as a promise
  // of healing. It shipped on a `selfCritBuff`.
  //
  // Not one stone: "Ice" is inside "Justice", "Sin" inside "Single", "Bow"
  // inside "Elbow", "Eye" inside "Eyeless". Every one of those is an identity
  // word that silently switches off a naming rule for any name that happens to
  // contain its letters, and the failures are invisible because the rule
  // returns "this name is fine".
  //
  // `\b` on both sides. A word that IS the essence's or the stone's own name
  // is still identity and still exempt -- which is the whole point of the
  // block above -- but only when it appears as a word.
  for (const w of ids) {
    if (w && String(w).length > 2) {
      const esc = String(w).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      n = n.replace(new RegExp(`\\b${esc}\\b`, 'ig'), ' ');
    }
  }
  ...
  // ROUND 207 -- and the same for the creature it bonds. "Call the Bee" on a
  // lizard is round 204's report; "Bear Rear" on a minotaur and "Rat Calls" on
  // a direbuck are what the census found beside it. Judged against the FAMILY,
  // not the creature's invented name -- a Panterimp is the wolf-family
  // creature, so "Wolf Calls" is right and the first census was wrong to
  // count it.
  //
  // AGAINST THE RAW NAME, NOT THE IDENTITY-STRIPPED ONE. This is the one rule
  // in the table where the identity exemption must not apply, and leaving it
  // on `n` made the census WORSE rather than better: "Call the Bee" on a Bee
  // essence has "Bee" stripped as identity, so the rule saw nothing and
  // returned "this name is fine" -- which is the exact fault round 204
  // reported. The exemption exists because a word that IS the essence's name
  // is identity rather than a mechanical claim; for a bonded creature it is
  // BOTH, and the claim is the half that matters.
  if (tags.familiar && namePromisesWrongBeast(name, tags.familiar)) return true;
  return false;
}

// ===========================================================================
// ROUND 205 (item 2.1) -- THE NAME IS JUDGED AGAINST THE FINISHED ABILITY.
//
// The user, on a Spear kit:
//
//   "Ability: [Healing Spike] (Spear) ... Holds every creature within 8 tiles
//    in place for 5 seconds ... 2.1) What about this has anything to do with
//    healing? The ability itself is good but the name is nonsense."
//
// `nameContradictsSpec` has gated `heal|healing|mend|...` on the `heals` tag
// since round 49 and it works. What it cannot do is answer a question about a
// spec that does not exist yet, and BOTH doors ask it too early:
//
//   * the socket path calls `pickAbilityName` partway through
//     `generateCategoryAbility`, before the lever twist, the mech pin, the
//     debuff and the cost pass -- any of which can move the template family.
//     "Restorative Hibernation" is the Pangolin sheet's Healing bucket landing
//     on a spec that finished as a `selfCritBuff`.
//   * the signature path never asks at all. `buildSignatureAbility` adopts
//     `entry.name` wholesale, and "Healing Spike" is authored in
//     essenceSignatures.js against `catKey: 'self_active_timefreeze'` -- the
//     contradiction is in the TABLE, so no work on the naming rules could
//     have caught it.
//
// Measured across 500 kits before this: 31 abilities whose name promised
// healing and whose mechanic does none. Both doors now re-ask at the bottom,
// with the finished spec in hand, which is the structural version of the fix
// rather than a third place that has to remember to check.
//
// When the name fails, `pickAbilityName` picks again -- the same door, now
// answering about the right ability. The essence keeps the signature's
// mechanic and its flavour text; what it loses is a name that was never true.
// ===========================================================================

/** Every name this process refused, so the rate is measurable from outside
 *  without re-deriving what "refused" means. Beside the thing counted, the way
 *  COMPOSED_DROPS is. */
export const NAME_RECHECK_REJECTS = [];

function renameIfContradicts(spec, cat, essDef, stoneId, comboSeed, usedNames, roll, opts, door) {
  if (!spec || !spec.name || !cat) return;
  const stone = STONE_THEMES[stoneId];
  const identity = [stone && stone.word, essDef && essDef.name].filter(Boolean);
  if (!nameContradictsSpec(spec.name, specTags(spec), identity)) return;
  const was = spec.name;
  const replacement = pickAbilityName(cat, essDef, stoneId, comboSeed, usedNames, spec, roll, opts);
  // A replacement that contradicts too is the ungated last resort at the
  // bottom of `pickAbilityName` answering, and swapping one false name for
  // another false name is churn. The original at least carries authored
  // flavour, so it stands and the reject is recorded either way.
  if (!replacement || nameContradictsSpec(replacement, specTags(spec), identity)) {
    NAME_RECHECK_REJECTS.push(`${door}: ${was} [${spec.template}] on ${essDef && (essDef.id || essDef.name)} -- no legal name`);
    return;
  }
  if (usedNames) { usedNames.delete(was); usedNames.add(replacement); }
  spec.name = replacement;
  NAME_RECHECK_REJECTS.push(`${door}: ${was} -> ${replacement} [${spec.template}] on ${essDef && (essDef.id || essDef.name)}`);
}
 ...
 *
 * ===== ROUND 205 (item 2.1) -- A WRAPPER, SO NO BRANCH CAN SKIP THE CHECK ===
 *
 * The first cut of this round put `renameIfContradicts` at the bottom of the
 * body, next to `spec.stats = ...`, and measured the heal-named non-healers
 * from 31 down to 4 -- with "Restorative Hibernation" (Pangolin's sheet
 * Healing bucket, landing on a `selfCritBuff`) still among them. It had never
 * been checked at all: the body has a dozen early returns, one per template
 * family that builds its own spec whole -- attrBoost, weaponMight,
 * twoHandWield, the corpse path, the perception, the aura -- and the bottom of
 * the body is not the bottom of the function.
 *
 * That is fault class 3 on this project's own list in its purest form: a rule
 * enforced at the places somebody remembered rather than at the door. So the
 * body is private and the exported name is this wrapper, which is the ONE exit
 * every branch passes through, including the ones a later round adds.
 */
export function generateCategoryAbility(catKey, essDef, stoneId, comboSeed, usedNames, opts = {}) {
  const spec = generateCategoryAbilityBody(catKey, essDef, stoneId, comboSeed, usedNames, opts);
  if (!spec || !spec.name) return spec;
  // The signature path does its own re-check AFTER adopting the authored name
  // and re-running the mechanical passes, so it opts out here rather than
  // being renamed twice against two different versions of the same spec.
  if (opts.skipFlavour) return spec;
  const cat = ABILITY_CATEGORY_BY_KEY[catKey];
  const roll = (salt, n) => stableHash(comboSeed + '|' + salt) % n;
  const before = spec.name;
  renameIfContradicts(spec, cat, essDef, stoneId, comboSeed, usedNames, roll, opts, 'socket');
  // A rename invalidates nothing computed from the name except the name
  // itself -- the ladder is seeded off it, so it is rebuilt rather than left
  // describing the ability that was not chosen.
  if (spec.name !== before) {
    spec.rankAspects = rankAspectsFor(spec);
    spec.stats = statsLineFor(spec);
  }
  enforceWeaponEssenceAttacks(spec, essDef);
  return spec;
}

/**
 * ===== ROUND 217 (item 1.1) -- A WEAPON ESSENCE DOES NOT CAST SPELLS =======
 *
 * The user, correcting round 216's reading of item 2:
 *
 *   "attacks can be spells or special attacks. Weapon essences shouldn't
 *    generate spells (except for the staff essence). So when I talk about
 *    attacks on a weapon essence those are special attacks."
 *
 * Round 216 tried to raise the special-attack count by biasing the COMPOSER,
 * moved the number 5%, and reported that as the ceiling. It was not the
 * ceiling; it was the wrong door. Measured properly: 44.7% of a weapon
 * essence's own attacks required the weapon, and the other 55% included
 * "Axe Split", "Double Swing" and "Chop" -- abilities built on `imbueStrike`,
 * which is a weapon strike by template, sitting in the kit as spells.
 *
 * They were not spells by any decision anyone made. They were spells because
 * `requiresWeapon` is set on the composer's path and most attack abilities do
 * not come down it; the category path sets it only when the CATEGORY declares
 * it. So the rule went in a place that sees one of the two routes.
 *
 * This is the door both routes converge on -- the composed path builds a
 * category and hands it here, and the category path arrives here directly --
 * so the rule is stated once and cannot be reached around.
 *
 * THE STAFF EXEMPTION is the user's, and it is the right shape: a staff is a
 * caster's weapon, so a Staff essence casting is a Staff essence being
 * itself. Keyed on the weapon IDENTITY rather than the essence name, because
 * Sceptre resolves to 'staff' too and a sceptre is no more a club than a
 * staff is.
 */
/**
 * ===== ROUND 217 (item 2) -- SPILLOVER =====================================
 *
 * The user, on the two ways forward round 216 put to them:
 *
 *   "spillover isn't a bad idea, solid synergy improvement"
 *
 * The other option was giving a weapon essence more of the kit's twenty
 * seats, which buys special attacks by taking variety away from the two
 * essences the player also chose. This buys them by making those essences
 * PARTICIPATE: a Fire essence in a Knife kit produces a burning dagger
 * technique, which is a fire ability and a knife ability at once.
 *
 * ONE IN THREE, and the fraction is the design. All of them would make the
 * other two essences into weapon essences and delete the reason to pick
 * them; none is where round 216 started. A third means a Fire+Knife player
 * sees fire that knows it is being held in a knife hand, and fire that does
 * not, and can tell which is which.
 *
 * Seeded off the socket, so it is stable across rebuilds like every other
 * pick in this generator, and refused when the kit's essences disagree about
 * the weapon -- `kitWeaponOf` already returns null for a deliberate
 * two-weapon build, and spilling one weapon over a kit that chose two would
 * be the generator picking a favourite.
 */
export function applyWeaponSpillover(spec, essDef, kitWeapon, seed) {
  if (!spec || !kitWeapon || spec.requiresWeapon) return spec;
  if (kitWeapon === UNARMED_ID) return spec;
  // Only an essence that is NOT itself a weapon: its own attacks are already
  // special attacks, by `enforceWeaponEssenceAttacks` directly above.
  if (weaponIdentityOf(essDef)) return spec;
  if (spec.category !== 'attack' || spec.kind !== 'active') return spec;
  if (stableHash(`${seed}|spill`) % 3 !== 0) return spec;
  spec.requiresWeapon = kitWeapon;
  spec.weaponSpillover = true;
  spec.stats = statsLineFor(spec);
  return spec;
}

export function enforceWeaponEssenceAttacks(spec, essDef) {
  if (!spec || spec.requiresWeapon) return spec;
  const w = weaponIdentityOf(essDef);
  // No weapon, the caster's weapon, or the unarmed essences -- all left alone.
  if (!w || w === 'staff' || w === UNARMED_ID) return spec;
  // Only an ATTACK. Round 112's rule stands: a prerequisite on something that
  // acts on nobody is a lockout with no upside, and the weapon suite asserts
  // that no buff, heal or restore carries one.
  if (spec.category !== 'attack') return spec;
  if (spec.kind !== 'active') return spec;
  spec.requiresWeapon = w;
  // The line has to SAY so -- test_round112_weapon asserts that every ability
  // requiring a weapon prints "requires a <weapon> in hand", and a silent
  // prerequisite is worse than none.
  spec.stats = statsLineFor(spec);
  return spec;
}

function generateCategoryAbilityBody(catKey, essDef, stoneId, comboSeed, usedNames, opts = {}) {
  ...
  const roll = (salt, n) => stableHash(comboSeed + '|' + salt) % n;
  // ROUND 189 -- a corpse ability is built whole by its own module: its
  // numbers are about bodies, not about the balance band a bolt is priced in.
  if (cat && cat.corpse) {
    const nm = cat.names[roll('corpsename', cat.names.length)].replace('{A}', essDef.name || 'Grave');
    const spec = corpseAbilitySpec(cat, essDef, {
      color, element: materialFor(stone, essDef).element, name: nm, stoneId,
    });
    // ROUND 190 -- a corpse ability ranks like any other (test_round79a: every
    // ability has a rank ladder); its ladder is the one its shape earns.
    spec.rankAspects = rankAspectsFor(spec);
    spec.stats = statsLineFor(spec);
    return spec;
  }
  ...
  let name;
  // ===== ROUND 203 (item 1) -- THE SELF-TRANSFORMATION, BUILT ============
  //
  // Three canon abilities, one row, three swaps (abilityCanon.js's
  // TRANSFORMATIONS). Which of the three this socket produces is chosen by
  // the essence's bound ATTRIBUTE where it has one, so a Spirit essence hands
  // you the sorcerer and a Power essence the combatant -- the swap the books
  // make is the swap the kit makes. An unbound socket falls to the seed.
  //
  // Everything else about it comes from the table rather than from here: the
  // card diffs `transformAtRank` rank by rank (abilityCard.js), the
  // proficiency is one of the user's five (proficiencies.js), and the cooldown
  // is canon's six hours at round 201's six-to-one -- sixty played minutes.
  if (cat.key === 'self_transformation') {
    const byAttr = { spirit: 'speciousSorcerer', power: 'counterfeitCombatant',
      recovery: 'instantAdept', speed: 'counterfeitCombatant' };
    const key = byAttr[opts.slotAttr]
      || TRANSFORMATION_KEYS[stableHash(`${comboSeed}|transform`) % TRANSFORMATION_KEYS.length];
    const row = TRANSFORMATIONS[key];
    const specTF = {
      name: 'x', kind: 'active', category: cat.category, template: 'transform',
      color, catKey, stoneId, essenceId: essDef.id, lever: null,
      // ROUND 203 -- `transformProf`, NOT `prof`. Round 202 made `spec.prof`
      // mean exactly one thing -- "this PASSIVE teaches a weapon proficiency,
      // permanently" -- and it is read by `_recomputeDerivedStats` and counted
      // by the round-202 suite. A transformation grants a proficiency FOR
      // THIRTY SECONDS, which is a different fact, and putting it under the
      // same key made a temporary grant look like a permanent one to every
      // reader of the field. One name, one meaning.
      transform: key, attr: row.attr, transformProfs: (row.profs || []).slice(), pool: row.pool,
      // Canon's own figure, at the played rate. `withCanonCooldown` keeps the
      // six beside the 3600 so the card can still quote the book.
      ...withCanonCooldown({}, 6),
      cost: { type: row.pool === 'health' ? 'mana' : row.pool, amount: 40 },
      duration: TRANSFORM_BY_RANK.iron.duration,
      desc: `For a little while you are the ${row.label.toLowerCase().replace(/^(specious|counterfeit|instant) /, '')} `
        + `you are pretending to be.`,
    };
    specTF.rankAspects = rankAspectsFor(specTF);
    specTF.name = pickAbilityName(cat, essDef, stoneId, comboSeed, usedNames, specTF, roll, opts);
    specTF.stats = statsLineFor(specTF);
    return specTF;
  }
  ...
  switch (cat.template) {
    ...
    case 'stacking': {
      ...
      // ROUND 190 (bug 1) -- "what does dodging attacks have to do with being
      // a juggernaut?" Nothing. Avoiding a blow is an evasive build's trigger;
      // anything without a swift or stalking lever takes the hit instead.
      if (spec.stackBuild === 'dodge') {
        const lv = ((effectiveMotif(essDef, stone) || {}).levers) || [];
        if (!lv.includes('swift') && !lv.includes('stalk')) spec.stackBuild = 'taken';
      }
      ...
      // ROUND 190 -- a boon BUILT by casting cannot also be SPENT by casting:
      // the runtime spends before it builds, so every cast paid out the single
      // stack the last one left. Built by casting, it spends when full.
      if (spec.stackBuild === 'cast' && spec.stackSpend === 'nextCast') spec.stackSpend = 'full';
    ...
    case 'weaponAffinity': {
      ...
      // ROUND 190 (update 8.2) -- A WEAPON STONE IN ANOTHER WEAPON'S ESSENCE.
      // The essence still decides what its abilities need (8.2.3); the stone's
      // weapon earns a way IN: it counts as the essence's weapon for abilities
      // (8.2 / 8.2.1), or the essence's weapon learns its swing speed (8.2.2).
      const stoneW = weaponIdentityOf(stone);
      const essW = weaponIdentityOf(essDef);
      if (stoneW && essW && stoneW !== essW && stoneW !== UNARMED_ID && essW !== UNARMED_ID && WEAPONS[stoneW]) {
        const sw = WEAPONS[stoneW];
        if (roll('kinship', 2) === 0) {
          spec.countsAs = { from: stoneW, as: essW };
          spec.desc += ` And a ${sw.name.toLowerCase()} in your hand counts as a ${w.name.toLowerCase()} for every ability that asks for one.`;
        } else {
          spec.affinityMode = 'speed';
          spec.rangePct = 0;
          const want = (sw.cooldown && w.cooldown) ? Math.round(((w.cooldown / sw.cooldown) - 1) * 100) / 100 : 0.2;
          spec.attackSpeedPct = Math.min(0.6, Math.max(0.1, want));
          spec.matchedTo = stoneW;
          spec.desc = `Long practice with ${phrase} teaches a ${w.name.toLowerCase()} the swing of a ${sw.name.toLowerCase()}: it strikes ${Math.round(spec.attackSpeedPct * 100)}% faster while you hold one.`;
        }
      }
  ...
  // ROUND 203 -- `kitRank` and `stoneId` are threaded through so the
  // transcendent roll in there can be rarer for a low-rank kit and seeded off
  // the socket. Both were already in scope at this call and simply were not
  // being passed; adding them here rather than reaching for globals keeps
  // `applyEssenceFlavour` a function of its arguments.
  if (!opts.skipFlavour) {
    applyEssenceFlavour(spec, essDef, stone, cat, roll, comboBase,
      { spine: opts.spine, kitRank: opts.kitRank || 'iron', stoneId });
  }
  ...
  // ===== ROUND 204 (item 2.10.2) -- AND IT IS REFUSED ON THE OTHER TWO =====
  //
  // The paragraph directly above describes this exact bug -- "deals no damage"
  // printed beside "escort strikes for 4 every 1.4s" -- and then fixed it for
  // one role out of the three that have `dmgShare: 0`. The user found the
  // other two: `[Ossify]` claimed "It never strikes." and its own numbers line
  // said the escort struck for 4 every 1.4s.
  //
  // So the rule is written against the PROPERTY rather than against the role
  // name: a summon whose whole clause is that it deals nothing cannot carry a
  // rider whose whole clause is that it deals something.
  if (spec.escort && spec.summonRole
      && SUMMON_ROLES[spec.summonRole] && SUMMON_ROLES[spec.summonRole].dmgShare === 0) {
    delete spec.escort;
  }
  applyRuntimeFieldNames(spec);
  ...
  // ROUND 203 -- ...AND `COOLDOWN_VARIES` IS A COOLDOWN.
  //
  // This test read "is it a positive number?", and a cooldown computed at use
  // is a string. So every composed ability that had just been given a Varies
  // cooldown three hundred lines above had it silently replaced with an
  // ordinary one here -- which is why the round-203 suite found `reciprocal`
  // stamped by nothing at all: the rule fired seventy-three times in two
  // hundred kits and was overwritten seventy-three times. The suite's
  // question was the right one to ask of any rule table: is every rule in it
  // reachable?
  ...
  // ===== ROUND 204 (item 2.2.3) -- A BUFF RIDER YOU CAN ACTUALLY SPEND =====
  //
  // The user, on a spear special attack that raised his cast speed: "Cast
  // speed is somewhat antithetical to a spear build."
  //
  // The flat seven-stat list had no gate at all, while the AUTHORED cast-speed
  // row two thousand lines up carries `leverGate: ['swift', 'shift']` -- the
  // rule existed and the composed path simply never asked. So the pool is
  // filtered by what the ability is:
  //
  //   castSpeed   only on something that casts. A special attack swings a
  //               weapon; shortening its cast time shortens nothing.
  //   critChance  on anything that deals damage, and nothing else: a crit
  //               chance on a shield is a statistic with no roll behind it.
  //   dodgeChance not on a weapon strike, which is the ability least able to
  //               use a defensive window it opens by attacking.
  //
  // The four attributes stay on everything, because an attribute is a
  // statement about the bearer rather than about the ability.
  if (cat.composed && cat._hasBuffRider && !spec.riderBuff) {
    const martial = !!spec.requiresWeapon
      || (spec.cost && spec.cost.type === 'stamina' && spec.category === 'attack');
    const casts = !martial && (spec.category === 'attack' || spec.castTime > 0
      || (spec.cost && spec.cost.type === 'mana'));
    const STATS = ['power', 'spirit', 'speed', 'recovery'];
    if (abilityDeals(spec)) STATS.push('critChance');
    if (casts) STATS.push('castSpeed');
    if (!martial) STATS.push('dodgeChance');
    spec.riderBuff = {
...
export function buildSignatureAbility(entry, essDef, stoneId, comboSeed, usedNames, opts = {}) {
  ...
  // ROUND 189 -- a corpse ability takes no lever twist: its payload is the
  // body, and a decay rider or a delayed scaling written on top of "the
  // nearest body bursts" would be a second ability wearing the first's name.
  // The drain's resource can be pinned by `mech`, so its lines are rebuilt.
  if (spec.corpse) {
    const cat2 = ABILITY_CATEGORY_BY_KEY[entry.catKey];
    const again = corpseAbilitySpec(cat2, essDef, { color: spec.color, element: spec.element,
      name: spec.name, stoneId: spec.stoneId, resource: spec.resource });
    again.desc = `${entry.desc} ${again.desc}`;
    again.signature = true;
    again.rankAspects = rankAspectsFor(again);   // ROUND 190 -- see generateCategoryAbility
    again.stats = statsLineFor(again);
    return again;
  }
  const roll = (salt, n) => stableHash(comboSeed + '|' + salt) % n;
  const essBase = essDef.base || 6;
  ...
  // ROUND 203 -- kitRank on the SIGNATURE path too. It was the fourth of the
  // four flavour calls and the only one still defaulting to iron, which made
  // one ability in three thousand come out gated inside a gold kit -- the
  // safe direction, and still a different rule for one path than the other
  // three.
  applyEssenceFlavour(spec, essDef, STONE_THEMES[stoneId], cat, roll, comboBase,
    { keepText: true, spine: opts.spine, kitRank: opts.kitRank || 'iron', stoneId });
  ...
  appendDebuffClause(spec);
  bindFamiliarCreature(spec, essDef, opts);
  stampVehicleBoons(spec, essDef);
  nameConjuredWeapon(spec);   // ROUND 112 -- see the function's own note
  // ROUND 205 (item 2.1) -- and the AUTHORED name is checked too. The
  // signature's name is adopted wholesale several hundred lines up, before
  // the mech pin, the lever twist, the debuff and the cost pass, so this is
  // the first point at which `specTags` is answering about the ability that
  // actually ships. "Healing Spike" is authored against
  // `catKey: 'self_active_timefreeze'` -- the contradiction is in the table,
  // and this door never asked. See `renameIfContradicts`.
  renameIfContradicts(spec, cat, essDef, stoneId, comboSeed, usedNames, roll, opts, 'signature');
  spec.rankAspects = rankAspectsFor(spec);
  spec.stats = statsLineFor(spec);
  return spec;
}
...
/**
 * ROUND 200 -- "The transports ... should also have a high chance of being an
 * ability with the vehicle essence."
 *
 * A boon is not an ability and does not take a socket: it is a list stamped on
 * whatever this essence's socket generated, and the runtime sums the lists
 * across the kit. See src/data/vehicleEssence.js for the twelve and for why
 * they are twelve rather than six.
 *
 * STAMPED OFF A STABLE HASH, like the mount flag above, and for the identical
 * reason: the kit is rebuilt on every socket, and a boon that changed between
 * two openings of the panel would read as a bug rather than as a build.
 */
function stampVehicleBoons(spec, essDef) {
  if (!spec || !isVehicleEssence(essenceIdOf(essDef))) return;
  const boons = vehicleBoonsFor(spec.key, stableHash(`${spec.key || spec.name || ''}|veh`));
  if (!boons.length) return;
  spec.vehicleBoons = boons;
  // Said out loud on the ability, because a bonus the player cannot see is a
  // bonus they will not build for. One clause per boon, in the boon's own
  // words -- the same shape `rankEffectLine` gives a rank rider.
  const said = boons.map(k => (VEHICLE_BOON_BY_KEY[k] || {}).text).filter(Boolean);
  if (said.length) spec.desc = `${spec.desc || ''} While you travel, ${said.join(', and ')}.`.trim();
}
...
export function pickSignatureAbility({
  ...
  kitRank = 'iron',   // ROUND 203 -- see the buildSignatureAbility call below
  ...
  for (const entry of list) {
    ...
    // ROUND 203 -- `kitRank` rides through to the signature builder's own
    // flavour pass. This was the FIFTH and last route an element comes through
    // and the one still rolling at iron inside a gold kit -- found by a census
    // that kept reporting exactly one gated transcendent ability per thousand
    // at silver and gold, which is the shape of a single unthreaded path
    // rather than of a random effect.
    const spec = buildSignatureAbility(entry, essDef, stoneId, `${seedStr}|${entry.name}`, usedNames,
      { slotAttr, usedParts, kitRank });
    if (spec && (!filter || filter(spec))) pool.push(spec);
  }
...
/**
 * ROUND 190 (7.5.6 / 6.1) -- THE FIRST ABILITY A CHARACTER GAINS.
 *
 * Always an attack -- a spell or a special attack -- and always the spammable
 * kind: cheap, near-zero cooldown, low damage, its cost climbing with the
 * streak. The essence's own signature attacks are tried first so a Might
 * character opens on a Might blow rather than the defensive "spell" in the
 * round-190 screenshot; if the essence has no signature attack, a generated
 * one in its voice.
 */
export function firstAbilityFor(essDef, o) {
  const isAttack = (c) => c && c.kind === 'active' && abilityDeals(c)
    && SPAMMABLE_TEMPLATES.includes(c.template);
  // A martial essence opens on a blow, a ranged one on a bolt: the lean the
  // build classes read (buildClasses.essenceLeanScore) picks the shape first.
  const lean = essenceLeanScore(essDef, weaponIdentityOf);
  const BOLTS = ['projectileBall', 'volley'];
  const leans = (c) => isAttack(c) && (lean > 0 ? !BOLTS.includes(c.template) : (lean < 0 ? BOLTS.includes(c.template) : true));
  let pick = pickSignatureAbility({ ...o, essDef, forcedKind: 'active', filter: leans })
    || pickSignatureAbility({ ...o, essDef, forcedKind: 'active', filter: isAttack });
  if (!pick) {
    for (const key of (lean > 0 ? ['self_active_aoe', 'ranged_damage'] : ['ranged_damage', 'self_active_aoe'])) {
      // ROUND 203 -- `kitRank` here too. Without it the opener rolled at iron
      // odds inside a gold kit and came out GATED, which is the safe direction
      // but is still a different rule for one socket than for the other
      // nineteen -- and a rule that holds for nineteen of twenty sockets is
      // the shape this project keeps finding at the twentieth.
      const c = generateCategoryAbility(key, essDef, null, `${o.seedStr}|opener|${key}`, o.usedNames,
        { slotAttr: o.slotAttr, kitRank: o.kitRank || 'iron' });
      if (isAttack(c)) { pick = c; break; }
    }
  }
  if (!pick) return null;
  const a = { ...pick };
  if (!a.spammable) makeSpammable(a);
  // The sentence is rebuilt from the finished numbers: the pick's own was
  // written for the ability before it was made spammable.
  // An authored signature keeps its authored sentence in the lead, whole.
  const authored = signaturesFor(essDef).find(e => e.name === pick.name);
  const flav = authored && authored.desc ? String(authored.desc).trim() : '';
  let mech = mechanicalDesc(a, { element: a.element || 'physical' });
  // Every kit opens on one of these, so the bolt's stock sentence would be one
  // line in sixteen of the whole roster (test_round62's 5% bar): three
  // phrasings, picked by name.
  if (a.template === 'projectileBall') {
    const el = ELEMENT_PHRASE[a.element || 'physical'] || 'force';
    // ROUND 204 (item 2.1.2) -- the user, on one of these: "This reads as
    // either AI slop or English as a second language. What does it mean? The
    // only bit of value in that whole statement is that it does 3 damage to
    // the first thing it touches."
    //
    // He is quoting the ONE alternative that buried its number in a clause
    // about snapping. All three now lead with what the ability does and to
    // what, and the variation is in the verb rather than in how long it takes
    // to reach the fact.
    const alts = [
      mech,
      `Throws a bolt of ${el} that deals ${a.base} damage to the first thing it hits.`,
      `Hits the first thing in its path for ${a.base} ${el} damage.`,
    ];
    mech = mech.replace(/^[^.]*\./, alts[stableHash(a.name || '') % alts.length].replace(/\.$/, '') + '.');
  }
  // ROUND 204 -- the same rule for the repeat clause. "It is ready again
  // almost at once" says nothing a cooldown line does not already say; what
  // the player needs is that repeating it is the point and that repeating it
  // costs more.
  const again = [
    'Each unbroken repeat hits harder and costs more.',
    'Repeat it without pausing and every use hits harder, for more.',
    'Used in an unbroken run, each strike gains damage and price together.',
  ][(stableHash(a.name || '') >>> 3) % 3];
  a.desc = `${flav ? flav + ' ' : ''}${mech} ${again}`;
  appendDebuffClause(a);
  assignAbilityCost(a, null);
  a.rankAspects = rankAspectsFor(a);
  a.stats = statsLineFor(a);
  a.opener = true;
  return a;
}
...
// ROUND 200 -- "No kit should have multiple mount abilities", and the argument
// above is already the right one: you are on a mount or you are not, so a
// second one is a wasted socket rather than a faster horse. Declared in
// mounts.js beside the rest of the mount rules and re-exported here so the
// three caps read as the one family of thing they are.
export { MOUNT_CAP };
...
export function buildCandidatePool({
  // ROUND 203 -- `kitRank` rides the same route every other kit-wide fact
  // does, and defaults to iron for the callers outside rebuildKnownAbilities.
  kitRank = 'iron',
  ...
  socketIndex = -1,
  kitUnarmed = false,
  // ROUND 215 -- the weapon the kit's essences declare, or null. Defaulted
  // for callers outside rebuildKnownAbilities (the guard and NPC kit
  // builders), which then behave exactly as they did before.
  kitWeapon = null,
  ...
  // ROUND 200 -- the mount seat, threaded and defaulted exactly as the two
  // above it. See the gate in `tryCat`.
  mountState = { count: 0, cap: MOUNT_CAP },
  confluenceName = null,
  ...
  // affine to.
  const weaponIdentity = weaponForAffinity(stone, essDef, kitWeapon);
  ...
  const tryCat = (cat, opts = {}) => {
    ...
    // spent on nothing.
    if (cat.isWeaponAffinity && !weaponForAffinity(stone, essDef, kitWeapon)) return false;
    ...
    const spec = generateCategoryAbility(cat.key, essDef, stoneId, comboSeed, usedNames,
      ...
      { slotAttr, avoidNames: new Set(pool.map(p => p.name)), spine, essenceIds, usedParts,
        heavyHandSource: opts.heavyHandSource, waterWalkSource: opts.waterWalkSource,
        // ROUND 203 -- the rank the kit is being BUILT at, which is what
        // decides how rare transcendent damage may be in it. A kit is rebuilt
        // whenever a stone moves, so a silver player re-socketing is rolling
        // at silver odds -- which is the mechanical reading of "at ranks below
        // Silver ... very rare" and "even at Silver and Gold ... rare".
        kitRank });   // ROUND 52, spine 53, essenceIds 76, usedParts 103, kitRank 203
  ...
  // ROUND 174 -- AND THE STONE'S OWN LEVER, FIRST.
  //
  // See FAMILY_LEVER: from round 109 until this round the composer read the
  // ESSENCE's motif and nothing else, so 95.8% of generated abilities matched
  // neither their stone's bias list nor its avoid list -- the stone had
  // stopped affecting what an ability does at all.
  //
  // FIRST in the list, deliberately. `composeAbility` reads its levers in
  // order (see the `forceLead` parameter and `spineFirst` in mergedBiasKeys),
  // so a socket now leads with what its stone means and elaborates with what
  // its essence is -- "a wolf, QUICKLY" rather than "a wolf, and also
  // something". It is ONE lever against the motif's three, so the essence
  // still supplies most of the character; the stone supplies the verb.
  //
  // De-duplicated rather than appended blindly: an essence whose motif already
  // carries `swift` and a Swift stone are agreeing, not stacking, and a lever
  // listed twice would weight it twice for no reason anybody chose.
  //
  // AND IT IS NOT DONE BY WIDENING THE POOL, which was this round's first
  // attempt and is recorded because it failed in an instructive way. Adding
  // the stone's lever to `composedLevers` and preferring it for the composer's
  // LEAD moved the measured figure by 1.4 points -- a lever contributes
  // several atoms and the lead is one of many -- and cost `test_round51_charters`
  // its "every essence offers a damage bolt" floor, because a stone's lever
  // crowding the pool is a stone deciding what an ESSENCE may do. The lever
  // list stays the essence's; what the stone gets is the TWIST, in
  // `applyEssenceFlavour`, which is the sentence the player actually reads and
  // the scalars that actually apply.
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (kind === 'active') {
        const comp = composeAbility({
          ...
          weaponIsEssence: !!weaponIdentityOf(essDef),
  ...
  const trySignature = (kind) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // ROUND 203 -- and the SEVENTH path. The socket signature seat: this is
      // where "Shadow Glide" came from, the one gated transcendent ability in
      // ninety silver kits. Seven call sites reaching one roll is exactly the
      // hand-off this project keeps being bitten by, which is why the guard is
      // a census over finished kits rather than a list of these lines.
      const spec = buildSignatureAbility(entry, essDef, stoneId, `${comboSeed}|sig|${entry.name}`, usedNames,
        { slotAttr, spine, kitRank });
      if (!spec) continue;
  ...
  if (pool.length < CANDIDATE_FLOOR) {
    ...
    for (let i = 0; i < ABILITY_CATEGORIES.length && pool.length < CANDIDATE_FLOOR; i++) {
      ...
      const topCat = ABILITY_CATEGORIES[(rotation + i) % ABILITY_CATEGORIES.length];
      // ROUND 190 -- the corpse templates are the six death essences'
      // signatures and nothing else's; the top-up must not hand a Renewal
      // essence a rotting body (test_round52_hot caught three).
      if (topCat.corpse) continue;
      if (tryCat(topCat, { topup: true })
...
export function rebuildKnownAbilities(slots, ESSENCES, locked = null) {
  ...
  // ROUND 215 (item 4.1) -- the weapon this kit's ESSENCES have committed to.
  //
  // Declared HERE, beside essenceIds, rather than beside kitUnarmed further
  // down: `processStones` is called before that point and reads it, so the
  // first placement put it in the temporal dead zone and every kit in the
  // game threw ReferenceError. kitUnarmed survives down there only because
  // nothing reads it before it is assigned.
  // Named `kitEssenceWeapon`, not `kitWeapon`: a local `const kitWeapon`
  // already exists further down this function meaning the SOCKET's weapon,
  // and the two collided in the temporal dead zone -- every kit in the game
  // threw ReferenceError, from a name clash between two different ideas that
  // had been given the same word.
  const kitEssenceWeapon = kitWeaponOf(essenceIds.map(id => ESSENCES[id]));
  ...
  // ROUND 190 (item 7) -- THE HIDDEN BUILD CLASS. Null until the confluence
  // forms: before it, the essences drive (the old scoring, untouched); after
  // it, the class does -- see buildClasses.js.
  const kitClass = _confDef && !_confDef.divine
    ? kitClassFor(confluenceNameForKit, essenceIds.map(id => ESSENCES[id]), weaponIdentityOf)
    : null;
  const auraState = { count: 0, cap: hasMultiAuraPassive(essenceIds) ? 2 : 1 };
  const perceptionState = { count: 0,
    cap: hasMultiPerceptionPassive(essenceIds, (slots.slotStones || []).flat()) ? 2 : 1 };
  ...
  // ROUND 190 -- a support or buff-secondary class may hold two more.
  if (kitClass && (kitClass.primary === 'support' || kitClass.secondary.includes('buff'))) buffState.cap = BUFF_CAP + 2;
  ...
  const addKnown = (key, ability, slotIndex, stoneId, essenceId) => {
    ...
    if (ability.template === 'aura' && _src && !ability.auraSignature) {
      ...
      if (_sig) {
        ...
        // twice.
        // ROUND 204 (item 2.7.2) -- the user, on `Yours carries Unity Mark:
        // what stands in your field is left [Balancing Ledger]`: "doesn't make
        // sense."
        //
        // It does not, and the reason is that `_sig.label` is the name of the
        // SIGNATURE -- a bookkeeping handle for which of an essence's aura
        // variants this is -- not the name of anything the player can see,
        // carry or cleanse. So the sentence introduced a proper noun that
        // appears nowhere else on the card and then said something true about
        // a different proper noun. The blurb was always the whole content.
        const _line = ` ${cap(String(_sig.blurb || '').trim())}.`;
        if (_sig.blurb && !String(ability.desc || '').includes(String(_sig.blurb).slice(0, 24))) {
          ability.desc = `${String(ability.desc || '').replace(/\s*$/, '')}${_line}`;
        }
      }
    }
    // ROUND 217 (item 1.1) -- THE ONE DOOR EVERY ABILITY GOES THROUGH.
    //
    // The first cut put this rule in `generateCategoryAbility`, which is the
    // door the composed and category paths converge on -- and moved the
    // weapon-essence special-attack share from 44.7% to 60.1%. The remaining
    // 40% were INNATE and SIGNATURE abilities, which are built by neither of
    // those routes. Two doors watched out of three is the same fault the rule
    // was written to fix, one level up.
    //
    // This line is where every ability in a kit is recorded, whatever built
    // it, so the rule is applied here and there is nowhere left to arrive
    // from. `essenceId` is the essence that OWNS the ability, which is the
    // right subject: a Fire ability sitting in a kit beside an Axe essence is
    // a fire ability, and only the Axe essence's own attacks are axe strokes.
    enforceWeaponEssenceAttacks(ability, ESSENCES[essenceId]);
    applyWeaponSpillover(ability, ESSENCES[essenceId], kitEssenceWeapon, `${key}|${essenceId}`);
  ...
  const processStones = (slotIndex, essDef) => {
    slots.slotStones[slotIndex].forEach((stoneId, arrIdx) => {
      ...
      // ROUND 190 (7.3 / 7.5.3-4) -- with a class, the active count is the
      // class's: twelve by default, fewer with a `passive` secondary, one more
      // for a pure damage class, always 8..14. The innates are all active, so
      // the stones make up the difference.
      const innateCount = essenceIds.length + (_confFormed ? 1 : 0);
      const budgetA = kitClass
        ? Math.max(0, Math.min(socketsTotal, kitClass.activeTarget - innateCount))
        : Math.max(0, Math.round(socketsTotal * shareA));
      ...
      if (!forced && socketsTotal > 0) {
        const fracA = socketsTotal ? budgetA / socketsTotal : shareA;
        const paceA = (socketsSeen + 1) * fracA;
        const paceP = (socketsSeen + 1) * (1 - fracA);
      ...
      // ROUND 190 (7.5.5) -- every kit carries an aura and a perception.
      if (kitCategoryCount('aura') < 1) owedFloors.push('aura');
      if (kitCategoryCount('perception') < 1) owedFloors.push('perception');
      ...
      const pool = buildCandidatePool({
        ...
        socketIndex: arrIdx, kitUnarmed, kitWeapon: kitEssenceWeapon,
        // ROUND 203 -- the rank this kit is being built at. `slots.kitRank` is
        // set by the caller (WorldScene passes the player's own rank); every
        // other caller of rebuildKnownAbilities -- the guard and NPC kit
        // builders -- leaves it undefined and gets iron, which is the rarest
        // transcendent band and the right default for a body that is not a
        // player character.
        kitRank: slots.kitRank || 'iron',
        unarmedState: {
        ...
        supportState: {
          ...
          // ROUND 190 -- a support CLASS is half support; the seat cap is for
          // everyone else.
          cap: kitClass && kitClass.primary === 'support' ? 10 : SUPPORT_SEAT_CAP,
        ...
        // ROUND 200 -- counted off what was TAKEN, like the two above. A mount
        // is a property of a bonded familiar rather than a template of its
        // own, so the count is `a.mount` and not a template name.
        mountState: { count: knownList.filter(a => a.mount).length, cap: MOUNT_CAP },
      ...
      let candidates = pool;
      // ROUND 190 (7.5.5) -- "These should not ever be in the first 3
      // abilities gained." The first three gained are always innates or the
      // first stone of a slot (stones are placed by the game, round-robin --
      // see WorldScene._autoSocketSlot), so those sockets never take one.
      const classKit = kitClass ? tallyKit(knownList, kitClass, abilityDeals) : null;
      if (classKit) classKit.primaryQuota = primaryQuotaFor(kitClass, kitClass.activeTarget);
      const earlySocket = slotIndex < 3 && (arrIdx === 0 || (slotIndex === firstSlot && arrIdx === 1));
      if (earlySocket) {
        const late = candidates.filter(c => c.template !== 'aura' && c.template !== 'perception');
        if (late.length) candidates = late;
      }
      // ROUND 103, BUG 6 -- the split yields to a floor at the last chance.
      if (!isRare && forced && !floorsCritical) {
        const filtered = candidates.filter(c => c.kind === forced);
      ...
      // ROUND 190 (7.2 / 7.5.7) -- THE CLASS AS A FLOOR. The pull in the
      // scorer is a preference; this makes the primary quota and the lean
      // hold. While the kit still owes its primary role and the active
      // sockets left are running out, an active socket takes only that role;
      // and an attack that leans the wrong way yields to one that leans right
      // while the kit is not yet majority its lean.
      if (!isRare && kitClass && classKit) {
        const role = { dps: 'attack', tank: 'defensive', healing: 'healing', support: 'support' }[kitClass.primary] || 'attack';
        const owedPrimary = classKit.primaryQuota - classKit.primary;
        const activesLeft = budgetA - stoneActive;
        if (owedPrimary > 0 && activesLeft > 0 && owedPrimary >= activesLeft - 3 && forced !== 'passive') {
          const fit = candidates.filter(c => c.kind === 'active' && abilityRoles(c, abilityDeals)[role]);
          if (fit.length) candidates = fit;
        }
        const leanOk = (c) => { const r = abilityRoles(c, abilityDeals);
          return !r.attack || (kitClass.lean === 'melee' ? !r.ranged : !r.melee); };
        const ahead = kitClass.lean === 'melee' ? classKit.melee > classKit.ranged + 1 : classKit.ranged > classKit.melee + 1;
        if (!ahead) {
          const right = candidates.filter(leanOk);
      ...
      for (const c of candidates) {
        ...
        const s = syn * 10 + dupKeyPull + (c.kind === lagging ? 15 : 0)
          ...
          + (classKit ? classPull(c, kitClass, classKit, abilityDeals) : 0)
      ...
      // Count what was actually KEPT: a locked ability overrides the pick.
      if (known[key].ability.kind === 'active') stoneActive++; else stonePassive++;
      socketOrdinal++;
  ...
  // ROUND 190 (7.5.6) -- "Players first gained ability is always a low cost,
  // low cooldown spell or special attack." The first essence bonded is the
  // slot the game recorded (`firstSlot`), else the lowest filled one.
  const firstSlot = (typeof slots.firstSlot === 'number' && slots.slotEssence[slots.firstSlot])
    ? slots.firstSlot : slots.slotEssence.findIndex(Boolean);
  for (let i = 0; i < ESSENCE_SLOTS; i++) {
    ...
    if (i === firstSlot) {
      const opener = firstAbilityFor(e, {
        seedStr: `${essId}|innate|${essenceIds.slice().sort().join(',')}|slot${i}`,
        knownList, usedNames, auraState, perceptionState, movementCount, buffState, absorbState,
        slotAttr: slotAttrs[i], usedParts: partsFor(i), usedShapes, shapeOf,
        kitRank: slots.kitRank || 'iron',   // ROUND 203
      });
      if (opener) {
        addKnown(`${i}:innate`, { ...opener, innate: true, essenceId: essId, stoneId: null, color: e.color }, i, null, essId);
        processStones(i, e);
        continue;
      }
    }
    ...
    const sig = pickSignatureAbility({
      ...
      // ROUND 203 -- the SIXTH path, found by the suite rather than by
      // reading: a gold kit was producing a GATED transcendent signature,
      // because `pickSignatureAbility` defaults kitRank to iron and this
      // caller -- the essence innate, one ability in twenty -- never passed
      // it. Threading five of six paths is the same bug as threading none,
      // one twentieth as often; the census is what makes that visible.
      kitRank: slots.kitRank || 'iron',
  ...
  if (confDef) {
    ...
    const innate = { ...confluenceInnateAbility(confDef, usedNames, spine,
      ...
      absorbState.count < absorbState.cap, slots.kitRank || 'iron'), innate: true, essenceId: 'confluence' };
  ...
  // ROUND 200 -- "No kit should have multiple mount abilities."
  //
  // HERE RATHER THAN IN THE POOL, and for the same reason the paragraph above
  // gives about kit synergy: the cap is a fact about the finished kit and the
  // first socket cannot be told what the sixteenth will bring. The pool seats
  // above (heavy hand, water walking) can gate at selection because each is a
  // whole CATEGORY that can simply be refused. A mount is not a category -- it
  // is a flag on a bonded familiar -- so refusing it at the pool would refuse
  // the familiar too, and a kit would lose a companion to a rule about riding.
  //
  // So the surplus loses the SADDLE, not the ability. The second and third
  // rideable familiar in a kit stay exactly what they were before round 200:
  // bonded familiars that fight beside you. Ordered by the list, so which one
  // keeps it is stable across rebuilds rather than depending on socket order.
  let mounts = 0;
  for (const a of knownList) {
    if (!a.mount) continue;
    if (++mounts > MOUNT_CAP) { a.mount = false; a.mountSurplus = true; }
  }

  // ===== ROUND 202 (item 2.8) -- THE WEAPON ESSENCE KNOWS ITS WEAPON =====
  //
  // The user: "Obviously this means that weapon essences should have a high
  // chance to roll related weapon proficiencies."
  //
  // A POST-BUILD SWEEP rather than a new socket category, for the reason the
  // mount cap above is one: adding a category would move the kit's 12 active /
  // 8 passive shape, which two hundred rounds of balance sit on. The
  // proficiency is STAMPED onto a passive the slot already holds -- preferring
  // the weapon passives, which already name a weapon and read as knowing it --
  // so it costs the kit nothing and shows up on a card the player was going to
  // read anyway.
  //
  // Seeded off the ESSENCE and the kit, not off a counter, so the same three
  // essences always produce the same answer: a Sword adept who reloads does
  // not discover they have forgotten how to hold a sword.
  for (const eid of new Set(slots.slotEssence.filter(Boolean))) {
    const grant = profGrantFor(eid);
    if (!grant) continue;
    // Seeded off the kit's own essence trio rather than a per-socket combo
    // seed, because the grant is a property of the ESSENCE being in this kit
    // at all, not of any one socket. `essenceIds` is the same list the whole
    // build is decided from, a few thousand lines up.
    // ===== ROUND 204 (item 3) -- AND BY BRONZE IT IS NOT A ROLL ==========
    //
    // The user: "A whip essence may not instantly provide a Dexterity Weapon
    // proficiency, but by bronze or silver it should be nearly impossible that
    // one of the abilities hasn't filled in that gap."
    //
    // Round 202's 80% was a single roll taken once, so one weapon kit in five
    // never learned its own weapon at any rank -- the character was not
    // untrained yet, they were untrained forever. The roll now decides WHEN
    // rather than WHETHER: at iron it is the 80% it always was, and from
    // bronze up the essence has certainly taught it.
    //
    // `kitRank` rather than a per-ability rank because the grant is a property
    // of the essence being in the kit (see the seeding note above), and the
    // kit is rebuilt whenever the character ranks up.
    const rankIdx = RANK_ORDER.indexOf(slots.kitRank || 'iron');
    const certain = rankIdx >= RANK_ORDER.indexOf('bronze');
    if (!certain && (stableHash(`${eid}|prof|${essenceIds.join(',')}`) % 1000) / 1000 >= grant.chance) continue;
    const slotIdx = slots.slotEssence.indexOf(eid);
    const mine = Object.values(known)
      .filter(e => e.slotIndex === slotIdx && e.ability && e.ability.kind === 'passive')
      .map(e => e.ability);
    if (!mine.length) continue;
    // Already trained by another essence in this kit? Two sources of one flag
    // is one wasted clause; the second essence keeps its passive unchanged.
    if (Object.values(known).some(e => e.ability && e.ability.prof === grant.key)) continue;
    const prefer = mine.find(a => a.template === 'weaponMight' || a.template === 'weaponAffinity')
      || mine[0];
    prefer.prof = grant.key;
    // ===== ROUND 204 (item 4) -- AND IT IS NAMED FOR WHAT IT NOW IS ========
    //
    // The sweep stamps the proficiency onto "whichever passive the slot
    // already holds", preferring a weapon passive -- and when the slot holds
    // none, the mastery line came out called "Wellspring of Life". That is
    // the name/effect mismatch the user spent this round's item 2 on, so the
    // carrier is renamed to what it has become.
    //
    // Only when the existing name has nothing to do with a weapon: a passive
    // already called "Spear's Fury" is a better name than "Spear Mastery" and
    // renaming it would lose the one the generator worked for.
    {
      const covered = PROFICIENCIES[grant.key].grants;
      const weaponish = new RegExp(`\\b(${covered.join('|')}|weapon|blade|haft|grip|edge|strike|swing)\\b`, 'i');
      if (!weaponish.test(prefer.name || '')) {
        // ===== ROUND 205 (item 1.1) -- NAMED AFTER THE ESSENCE'S WEAPON ====
        //
        // The user: "Ability: [Hammer Mastery] (Axe)". This line read
        // `covered[0]` -- the FIRST weapon the proficiency grants -- so every
        // `power` mastery in the game came out a hammer whether the socket
        // held a hammer or an axe, and the card printed the essence's real
        // name in brackets two words away from the contradiction.
        //
        // `masteryWeaponFor` asks the ESSENCE instead, and answers null when
        // the essence names no weapon the proficiency covers rather than
        // guessing -- in which case the proficiency's own label is the honest
        // heading. See its note in proficiencies.js.
        const wid = masteryWeaponFor(eid, grant.key);
        const noun = (wid && (WEAPONS[wid] || {}).name) || PROFICIENCIES[grant.key].label;
        const want = `${noun} Mastery`;
        if (!usedNames.has(want)) { usedNames.delete(prefer.name); prefer.name = want; usedNames.add(want); }
      }
    }
    // The description STATES THE MECHANIC, which is this project's standing
    // rule and is the whole of why the clause is appended rather than implied.
    const line = ` It also teaches you ${PROFICIENCIES[grant.key].short}: `
      + `${PROFICIENCIES[grant.key].blurb}.`;
    if (!String(prefer.desc || '').includes(PROFICIENCIES[grant.key].short)) {
      prefer.desc = `${String(prefer.desc || '').replace(/\s*$/, '')}${line}`;
    }
    // ===== ROUND 204 (item 4) -- AND ITS LADDER IS REBUILT ================
    //
    // `prof` is what makes an ability the mastery line, and this sweep is the
    // thing that stamps it -- but `rankAspects` was computed back in
    // `generateCategoryAbility`, before `prof` existed, so the ability climbed
    // whatever ladder it would have climbed without one. Measured: the
    // proficiency clause landed and the bronze rung was still the strike
    // ladder's "15% of the damage carries to two more foes nearby".
    //
    // The order cannot be reversed -- the sweep needs the finished kit to know
    // which passive to stamp -- so the ladder is rebuilt here, which is the
    // one line that makes items 3 and 4 the same ability.
    prefer.rankAspects = rankAspectsFor(prefer);
    prefer.stats = statsLineFor(prefer);
  }

  // ===== ROUND 205 (item 4.1) -- THE ARSENAL'S OWN PASSIVE ================
  //
  // The user, having ruled that two weapon essences always make an Arsenal:
  //
  ...
  //          rewarded with buffs from both."
  //   4.1.3 "This gives the Arsenal Confluence a specific identity."
  //
  // A POST-BUILD SWEEP, the same shape as round 202's proficiency sweep
  // directly above and for the same two reasons: it costs the kit none of its
  // 12 actives / 8 passives, and it needs the FINISHED kit -- the set of
  // weapons to share across is the union of what the masteries cover, and the
  // masteries are stamped by the sweep above this one. Order is load-bearing.
  //
  // "Always", which is why this is a sweep and not a signature. A confluence
  // signature pool offers five and the socket picks one; an identity that
  // arrives four times in five is not an identity.
  //
  // THE WEAPON SET IS THE PROFICIENCIES' GRANTS, not the essences' own
  // weapons, because that is what the user's example says: "Hammer Mastery and
  // Spears fury apply their effects to both Axes and Spears" -- a Hammer
  // essence's mastery covers the axe as well, and the axe is in the set.
  //
  // ===== ROUND 205b -- AND THE GATE IS THE ESSENCES, NOT THE NAME =========
  //
  // The user, on the one trio the canon keeps out of Arsenal:
  //
  //   "the specific cold, hammer, sickle version of the desolate confluence
  //    should have a similar guaranteed ability to have sickle and hammer
  //    essence abilities benefit one another."
  //
  // The first cut tested `confDef.name === 'Arsenal'`, and that was the wrong
  // hook: the sharing is a fact about a player carrying two weapon essences,
  // and Cold + Hammer + Sickle is exactly that player wearing a name the sheet
  // gave them. Tying a mechanic to a NAME means every future canon row that
  // happens to hold two weapons quietly loses it, which is this project's
  // fault class 3 dressed as a confluence.
  //
  // So the test is the count, which also settles the other direction without a
  // second rule: Myriad + Shield + Sword reaches Arsenal by the canon row with
  // ONE weapon essence, and one essence has nothing to cross-apply to. That
  // confluence keeps its name and skips the passive rather than carrying a
  // sentence about sharing between a sword and the sword.
  if (confDef && countWeaponEssences(slots.slotEssence.filter(Boolean)) >= 2) {
    const arsenalWeapons = new Set();
    for (const eid of new Set(slots.slotEssence.filter(Boolean))) {
      const key = ESSENCE_PROFICIENCY[eid];
      if (key && PROFICIENCIES[key]) for (const w of PROFICIENCIES[key].grants) arsenalWeapons.add(w);
    }
    if (arsenalWeapons.size >= 2) {
      const confPassives = Object.values(known)
        .filter(e => e.slotIndex === 3 && e.ability && e.ability.kind === 'passive')
        .map(e => e.ability);
      // The confluence slot first, because this is the CONFLUENCE'S identity
      // and a card that says so under the confluence's own name is the one the
      // player will go looking for. Any passive otherwise -- a kit whose
      // confluence slot rolled four actives still gets the identity.
      const any = Object.values(known)
        .filter(e => e.ability && e.ability.kind === 'passive' && !e.ability.arsenal)
        .map(e => e.ability);
      const carrier = confPassives.find(a => !a.arsenal) || any[0];
      if (carrier) {
        carrier.arsenal = true;
        carrier.arsenalWeapons = [...arsenalWeapons];
        // Named for what it now is, on the same terms as the mastery rename
        // above: only when the existing name has nothing to do with the idea.
        //
        // ROUND 205b -- and it is named for the confluence it belongs to. A
        // Desolate that shares its training is not "The Whole Arsenal"; the
        // card prints "(Desolate)" in brackets on the line above, and a name
        // arguing with it is the same mismatch item 1.1 was about. Arsenal
        // keeps the plain form, because "Arsenal Arsenal" is not a name.
        if (!/\b(arsenal|armou?ry|rack|every weapon|all weapons)\b/i.test(carrier.name || '')) {
          const want = confDef.name === 'Arsenal' ? 'The Whole Arsenal' : `${confDef.name} Arsenal`;
          if (!usedNames.has(want)) { usedNames.delete(carrier.name); carrier.name = want; usedNames.add(want); }
        }
        // THE DESCRIPTION STATES THE MECHANIC. This project's standing rule,
        // and the reason the weapons are listed by name rather than described
        // as "the weapons your essences cover" -- the player cannot count a
        // set they cannot see.
        const names = [...arsenalWeapons].map(w => ((WEAPONS[w] || {}).name || w).toLowerCase()).sort();
        const line = ` Every weapon your essences reach -- ${listPhrase(names)} -- `
          + `counts as every other: a bonus you own with one of them, you own with all of them.`;
        if (!/counts as every other/.test(String(carrier.desc || ''))) {
          carrier.desc = `${String(carrier.desc || '').replace(/\s*$/, '')}${line}`;
        }
        // And its ladder is rebuilt, for the reason the mastery's is: `arsenal`
        // is what selects the arsenal line and this sweep is what stamps it.
        carrier.rankAspects = rankAspectsFor(carrier);
        carrier.stats = statsLineFor(carrier);
      }
    }
  }

  // ===== ROUND 206 -- THE KIT'S OWN SYNERGY, SPREAD ACROSS ITS ABILITIES ==
  //
  // The user, choosing between one passive and many clauses:
  //
  //   "Spread around many abilities, I worry about the wording but it's the
  //    right way to build a truly fascinating ability engine."
  //
  // and the shape they asked for:
  //
  //   "sword abilities that trigger bleed and fire, fire abilities that
  //    consume stacks of bleed ... and bleed abilities that drain life from
  //    burning enemies"
  //
  // This is the first third of that -- `seed`, the only verb wired this round
  // (see kitSynergy.js for why the other four wait). An ability whose essence
  // WANTS a token another essence in the kit OFFERS gains that essence's
  // condition, at a chance and duration seeded off the socket.
  //
  // A SWEEP, like the proficiency sweep and the arsenal sweep above it, and
  // for the same reason all three are: it needs the finished kit. The resolver
  // is pure -- it reads the kit and returns a plan -- and the stamping is
  // here, which is what lets a probe measure the plan without building the kit
  // twice.
  for (const plan of resolveKitSynergy(known, essenceIds)) {
    const ent = known[plan.key];
    const a = ent && ent.ability;
    if (!a || !plan.text) continue;
    // ROUND 207 -- the gate verb. A predicate on the moment plus a payload,
    // resolved per cast by the scene against `_synergyCtx`. See
    // synergyGates.js for why this is one seam rather than fifteen.
    if (plan.verb === 'gate') {
      a.synergy = { verb: 'gate', from: plan.sourceEssence, sin: plan.sin || null };
      a.synergyGate = { gate: plan.gate, effect: plan.effect, pct: plan.pct, sin: plan.sin || null,
        // ROUND 213 -- a gate planted as an ALIAS'S READER. Marked because it
        // is a different kind of gate: an ordinary one is rolled off the
        // stone and, in a Sin kit, is always a sin. This one exists to read
        // the element another ability lends, so it answers to that element
        // rather than to a transgression, and a check that asserts every gate
        // in a Sin kit is a sin has to be able to tell them apart.
        fromAlias: !!plan.fromAlias };
      if (!String(a.desc || '').includes(plan.text)) {
        a.desc = `${String(a.desc || '').replace(/\s*$/, '')} ${plan.text}`;
      }
      continue;
    }
    // ROUND 213 -- the three that were pending since round 206.
    //
    // Each gets its own branch with its own `continue`, and the reason is the
    // line directly below: anything that falls past here is stamped as a
    // SEED, reading `plan.condition.key` off a plan that has no condition. A
    // new verb added without a branch would not error -- it would silently
    // become a seed with an undefined key, which is the quietest kind of
    // wrong this generator can produce.
    if (plan.verb === 'alias' || plan.verb === 'transfer' || plan.verb === 'alternate') {
      a.synergy = { verb: plan.verb, from: plan.sourceEssence || null };
      if (plan.verb === 'alias') a.synergyAlias = { from: plan.from, to: plan.to };
      if (plan.verb === 'transfer') a.synergyTransfer = { from: plan.from, to: plan.to };
      if (plan.verb === 'alternate') a.synergyAlternate = { poles: plan.poles.slice(), pct: plan.pct };
      if (!String(a.desc || '').includes(plan.text)) {
        a.desc = `${String(a.desc || '').replace(/\s*$/, '')} ${plan.text}`;
      }
      continue;
    }
    a.synergy = { verb: plan.verb, from: plan.sourceEssence, token: 'dot' };
    // The same shape `a.debuff` carries, so `_applyAbilityDebuff` applies it
    // with the machinery that already exists rather than a second path.
    a.synergyDebuff = { key: plan.condition.key, chance: plan.chance,
      duration: plan.duration, stacks: 1 };
    if (!String(a.desc || '').includes(plan.text)) {
      a.desc = `${String(a.desc || '').replace(/\s*$/, '')} ${plan.text}`;
    }
    // The numbers line carries it too -- see debuffStatsFragment's round-206
    // note on why describing it and not counting it would be the same split
    // round 57 fixed, facing the other way.
    a.stats = statsLineFor(a);
  }
  ...
  const counts = {
    ...
    // ROUND 202 -- so 2.8 is checkable from outside, exactly as `mounts` is.
    proficiencies: knownList.filter(a => a.prof).length,
    ...
    // ROUND 200 -- so the cap is checkable from outside, exactly as `buffs` is.
    mounts: knownList.filter(a => a.mount).length,
  };
  return { known, confluence: confDef, counts, spine: spineInfo, buildClass: kitClass };
}
